...
def j1d_params(E=20000.0, sigY=20.0, H=0.0, res_frac=0.0, H_res=None,
               law="linear", K_inf=None, alpha_f=None):
...
def j1d_return(sig_tr, alpha_n, p, newton_tol=1.0e-14, newton_max=50):
...
def j1d_step(state, deps, p, tau, dt, variant="tt"):
    ...
    Returns (sig, D, new_state, info).
    """
    E = p["E"]
    beta = dl_beta(tau, dt)
    deps = np.asarray(deps, float)
    sig_tr = state["sig"] + E * deps                      # viscoplastic elastic predictor

    if variant == "tt":
        # TWO-TRACK: the inner material never sees tau; it walks the total strain path inviscidly.
        sig_tr_in = state["sig_in"] + E * deps
        sig_bar, alp_bar, Dep, plastic = j1d_return(sig_tr_in, state["alp_in"], p)
        sig = (1.0 - beta) * sig_tr + beta * sig_bar
        D = (1.0 - beta) * E + beta * Dep
        ...
        sig_bar, alp_bar, Dep, plastic = j1d_return(sig_tr, state["alp"], p)
        sig = (1.0 - beta) * sig_tr + beta * sig_bar
        D = (1.0 - beta) * E + beta * Dep
        new = {"sig": sig, "eps": state["eps"] + deps,
    ...
    info = {"beta": beta, "sig_tr": sig_tr, "sig_bar": sig_bar,
