...
# WHY THIS LIST IS NOT LONGER, i.e. the coverage boundary of this file.
#
# The same `exit(-1)`-on-null-wrapper pattern was fixed in 17 SOE classes total, but
# this list is complete at 10 and CANNOT be extended to the rest: the other seven are
# not reachable from any serial `system` verb, so there is no deck this gate could
# write that would construct them.
#   - MumpsSOE, MPIDiagonalSOE          -- compiled only into OpenSeesSP/MP/PyMP
#   - DistributedDiagonalSOE,           -- compiled into the serial build, but no
#     DistributedSparseGenRowLinSOE,       `system` verb constructs them (they are
#     SparseGenRowLinSOE, ItpackLinSOE     built by the parallel machinery / unwired)
#   - PetscSOE                          -- compiled by NO build in this tree
#                                          (`#add_subdirectory(petsc)` is commented
#                                          out); reviewed only, never even built
# Those rows are compile-checked by `build.bat OpenSeesPy OpenSeesSP OpenSeesMP`, and
# that is the whole of their verification -- see LEDGER_quirks.md, which tiers all 17
# explicitly. If you are tempted to "finish" this list: check first that the class can
# actually be constructed from a script, and do not confuse compiling with running.
#
# A SEPARATE and still-OPEN family needs an MPI harness, not this file: the
# `Distributed*` / `MumpsParallelSOE` `getB()` overrides have no null guard at all and
# fault instead of exiting. `system Mumps` builds MumpsParallelSOE, so its `printB` is
# still unsafe on an unsized SOE even though MumpsSOE::getB here was fixed.
