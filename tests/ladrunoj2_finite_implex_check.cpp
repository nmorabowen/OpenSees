// Standalone cross-check of the LadrunoJ2Finite IMPL-EX (Δγ-extrapolation) path,
// using ONLY the OpenSees-free kernels — NO OpenSees build required. It mirrors, line
// for line, the math of SRC/material/nD/LadrunoJ2Finite.cpp::setTrialF in the
// useImplex branch (flow-direction recovery N=Δεᵖ/Δγ, co-rotation of the committed
// N_n by R=polar(f_Δ), and the elastic explicit stress at the frozen plastic strain).
// The engineering<->tensor Voigt bridging + the elastic-law shear factors are exactly
// what the 3×3-tensor numpy oracle cannot catch. The python driver
// tests/test_ladrunoJ2_finite_implex_cpp.py compiles this with g++, runs the SAME path
// through tests/ladrunoj2_finite_implex_reference.py, and compares.
//
//   g++ -O2 -std=c++17 -I ../SRC/material/nD ladrunoj2_finite_implex_check.cpp -o lj2fimx
...
struct Mat {
  ...
  double dGamma_n, Nflow_n[6];                 // IMPL-EX extrapolation history
  void init(const Params& pp){ p=pp; nB=pp.nBack;
    ...
    dGamma_n=0; for(int i=0;i<6;i++) Nflow_n[i]=0; }
  // setTrialF (IMPL-EX) + commit; writes the EXPLICIT Cauchy sig6, committed ebarP,
  // committed total backstress tot6.
  void step(const double F[9], double sig6[6], double& ebarP, double tot6[6]) {
    ...
    ladruno_j2_kernel::returnMap(p,st,ZERO6,ebarP_n,at,s6,D,epsP,eb,al,dG);  // implicit truth
    double J=mat3_det(F), invJ=1.0/J;

    // recover flow direction (current frame) for the next extrapolation
    double Nflow_t[6];
    if (dG>0.0){ double inv=1.0/dG; for(int i=0;i<6;i++) Nflow_t[i]=epsP[i]*inv; }
    else       { for(int i=0;i<6;i++) Nflow_t[i]=0.0; }

    // IMPL-EX explicit stress: elastic stress at εᵉᵗʳ − Δγ̃·Ñ_n (Δγ̃ = dGamma_n)
    double Npf[6]; rotateSym6(R,Nflow_n,Npf);
    double lam=p.K - 2.0*p.G/3.0;
    double ee[6]; for(int i=0;i<6;i++) ee[i]=st[i]-dGamma_n*Npf[i];
    double tre=ee[0]+ee[1]+ee[2];
    double sx[6];
    sx[0]=lam*tre+2.0*p.G*ee[0]; sx[1]=lam*tre+2.0*p.G*ee[1]; sx[2]=lam*tre+2.0*p.G*ee[2];
    sx[3]=2.0*p.G*ee[3]; sx[4]=2.0*p.G*ee[4]; sx[5]=2.0*p.G*ee[5];
    for(int i=0;i<6;i++) sig6[i]=sx[i]*invJ;

    // commit the IMPLICIT history + refresh extrapolation (Δγ_n, N_n)
    double ein[6]={eng[0]-epsP[0],eng[1]-epsP[1],eng[2]-epsP[2],
                   eng[3]-2*epsP[3],eng[4]-2*epsP[4],eng[5]-2*epsP[5]};
    double Beu[9]; be_from_hencky_voigt(ein,Beu);
    for(int i=0;i<9;i++){Fn[i]=F[i];Be_n[i]=Beu[i];} ebarP_n=eb;
    for(int k=0;k<nB;k++) for(int i=0;i<6;i++) alpha_n[k][i]=al[k][i];
    dGamma_n=dG; for(int i=0;i<6;i++) Nflow_n[i]=Nflow_t[i];
    ebarP=ebarP_n;
