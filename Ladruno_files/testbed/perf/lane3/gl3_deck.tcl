# ADR-75b G-L3 — the re-entry gate for Lane 3 (threaded element assembly).
#
...
nDMaterial J2Plasticity 1 $K $G 250.0 400.0 0.1 1000.0
...
if {$pid == 0} { puts "GL3_MODEL nx=$NX ny=$NY nz=$NZ dof~$NDOF np=$np loadf=$LOADF nstep=$NSTEP prof=$PROF" }
...
if {$pid == 0} { puts "GL3_RESULT ok=$ok wall_ms=[expr {$t1-$t0}] dof~$NDOF np=$np" }
if {$k1 == $NZ} { puts [format "GL3_TIP pid=%d ux=%.17e" $pid [nodeDisp $tip 1]] }

if {$PROF} {
    set out "gl3_prof"
    if {[info exists env(GL3_OUT)]} { set out $env(GL3_OUT) }
    profiler report "${out}.rank${pid}.h5"
    if {$pid == 0} { puts "GL3_REPORT ${out}.rank${pid}.h5" }
}
wipe
