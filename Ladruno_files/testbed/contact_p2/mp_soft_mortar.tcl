# ADR-78 P2 refusal deck, MORTAR lane. This is the lane where P2 is the ONLY
# guard: P1's slave-mass abort scans NTS contacts exclusively, so pre-P2 this
# deck ran to completion on 2 ranks with a partitioned SOFT=2 declaration
# (inert under this static integrator, SILENTLY mis-sized under the explicit
# one it exists for). EXPECTED post-P2: the named FATAL refusal at handle();
# the tail line must never print.
wipe
...
if {$pid == 0} {
    ...
    contactSurface 2 -slave-segments 4 11 12 13 14
    contact 1 1 2 -mortar -epsN 1.0e6 -soft 0.1 -outward 0.0 0.0 1.0
...
set ok [analyze 1]
puts [format "P2SOFTMORTARMP pid=%d analyze=%d SHOULD-NOT-PRINT" $pid $ok]
wipe
