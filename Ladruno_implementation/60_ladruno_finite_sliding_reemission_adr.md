...
| R3 | HIGH (curved) | `orientDir` not persisted; re-derived from reference each handle. Planar-safe (+`-outward` safe), but a sharp convex ridge can flip the normal → silent pass-through. The convex-ridge gate test is absent. **Full fix needs consistent-winding normals or nodal smoothing (ADR-47), not just persistence.** | **RESOLVED → [[63_ladruno_nodal_normal_smoothing_adr]]** (2026-06-30, P0+P1 built+validated). `-smoothNormal` gives a GLOBAL surface-sign averaged nodal-normal field ⇒ the per-slave ridge flip is structurally impossible (convex-ridge gate passes). Interim for un-migrated models stays `-outward`. |
| R5 | MED | BLOCKER-SLIDE-OFF absent (relies on GC + projection; a stale adapter can persist ≤ floor; an edge clamp can hold spurious traction). | **RESOLVED — verified already-handled, no code change.** Empirical probe (`Ladruno_scripts/_probe_r5_slideoff.py`): a frictional slave flung off the +x END of a finite strip under `-reemit` departs cleanly — contact force → 0 the instant it leaves (the narrow phase gates on penetrating-AND-in-bounds via `evalSegment`, so an out-of-bounds slave gets zero force), it falls freely under the press (no phantom normal traction), and its x-velocity is retained (no phantom friction). The migration trigger + friction GC drop the stale adapter within a bounded window, and D4 fresh-slot re-engagement keeps any re-pairing continuous. The "edge clamp holds spurious traction" never materialized — `project()` returns out-of-bounds UNclamped and `evalSegment` rejects it. (The frozen non-`-reemit` path shows a stale B3 force *snapshot* + the pre-existing pass-through, both of which `-reemit` incidentally fixes.) Regression guard: `tests/test_adr60_reemit_p4_slideoff.py`. |
...
| R8 | LOW | `runawayGuardFired()` never surfaced (dead warning). | **RESOLVED — intentionally not surfaced (documented).** As implemented the guard fires whenever the 1/99-percentile clamp moves a bound, i.e. for ANY mesh with >100 segment-centroids (the tails are clipped by design), so it is not a clean "a node ran away" signal — auto-warning on it would spam normal large models. The safety Finding-3 actually wanted (the grid never collapsing during instability) is provided by `clipPct=0` on the deformed feed, not by a warning. The accessor stays available for debugging/tests; the rationale is in a `// ADR-60 R8` comment at the Grid-construction site + `LEDGER_quirks.md`. |

**Exposed combo today:** ~~`-reemit` + `-mu` on a **curved** master (R3)~~ **RESOLVED by ADR-63 `-smoothNormal`**
(global-sign nodal-normal field; the ridge flip is structurally impossible). Until ADR-63 P2 composes friction +
re-emit + facet-ownership under smoothing, the belt-and-suspenders interim on curved masters remains `-outward`.
The re-meshable-master friction-aliasing case is guarded by R1 (membership fingerprint). Flat / `-outward` masters —
the shipped + tested path — are unaffected.
...
(`tests/test_adr60_reemit_p3_cadence.py`); full battery (now 145 tests) green ⇒ OFF byte-identical. Then **R5 + R8
(#453)** — R5 verified empirically already-handled (slide-off departs cleanly: force→0, free fall, no phantom
traction; regression test `tests/test_adr60_reemit_p4_slideoff.py`) and R8 documented as intentionally-not-surfaced
(noisy 1/99-percentile signal; clip-off is the real safety) ⇒ both doc + test, no logic change. **Backlog now CLOSED
except R3** (curved-master `orientDir` — deferred by decision to a dedicated ADR-47 #4a nodal-normal-smoothing pass;
`-outward` is the documented interim for curved/non-planar masters).
