...
| 4a | **Averaged nodal-normal smoothing** (smooth `N(X)` field) — **PULLED FORWARD → [[63_ladruno_nodal_normal_smoothing_adr]]** (2026-06-30). Pulled early not for the P3 chatter gate but to resolve **ADR-60 R3** (curved-master `orientDir` ridge flip → silent pass-through). P0+P1 BUILT (NTS lane; `-smoothNormal` opt-in, OFF byte-identical); resolves R3's sign flip + the NTS slice of Q-NORMAL (C0 normal). | ~~Pull earlier than #4 if the P3 chatter gate fails~~ **DONE** — see ADR-63. | ADR-41 Q-NORMAL (TG §5.1.1) |
