...
project: Ladruno
status: validated A–E + perf (2026-06-02); hardened after adversarial review + ASDSteel1D hysteresis cross-check (2026-06-03)
owner: nmora
tags:
  - validation
  - material
...
>   converge" (§4.4).
>
> **Hysteresis cross-check follow-up (2026-06-03).** A review of the *cyclic* ASDSteel1D
> comparison found the original E-figure plotted **only the damaged loops**, which diverge from
> ASDSteel1D and could read as "the models disagree." Fixes: (1) added the **undamaged** cyclic
> overlay (E3a) — matched-backbone loops that genuinely **coincide** (~3.8% resid) — as the real
> apples-to-apples check; (2) added a **rough calibration** of Lemaitre $(p_D,r)$ to ASDSteel1D's
> `-fracture` onset+energy (D2/E3b), which recovers peak (544 vs 545) and energy (63.8 vs 63.8),
> leaving only the by-design softening-*shape* difference; (3) annotated the cause (both couple
> $\sigma=(1-d)\tilde\sigma$ identically; the *evolution* laws differ — necking $e_{pl}$ vs
> accumulated-$p$ energy release). §5/§6.
>
...
| **D2** | rough **calibration** to ASDSteel1D `-fracture` (onset + energy) | peak **544 vs 545**, $\int\sigma\,d\varepsilon$ **63.8 vs 63.8**; residual = softening *shape* only |
| **D3** | IMPL-EX completion (both `-implex`, full push-to-rupture, $D>0.5$) | both complete |

The middle panel is the convergence curve (log–log); the right panel (D2) overlays the
ductile-fracture coupons.

> ### Why the damaged curves differ — and how far calibration closes it (the hysteresis question)
> Both models couple damage into stress the **same way** — isotropic stiffness *and* strength
> degradation, $\sigma=(1-d)\,\tilde\sigma$, $C=(1-d)\,\tilde C$ (ASDSteel1D:
> [`ASDSteel1DMaterial.cpp:2272`](../../SRC/material/uniaxial/ASDSteel1DMaterial.cpp); Lemaitre: §1.1).
> So the *coupling* is identical; what differs is the **damage-evolution law**:
>
> | | driving variable | onset | post-peak shape |
> |---|---|---|---|
> | **ASDSteel1D `-fracture`** | plastic strain $e_{pl}$ (`epl += sg·Δλ`, [line 415](../../SRC/material/uniaxial/ASDSteel1DMaterial.cpp)) | **necking** threshold $e_{upl}=e_u-s_y/E$ | $d=1-e^{-s_y(e_{pl}-e_{upl})/G}$ — **sharp**, necking-driven |
> | **Lemaitre** | accumulated $p$ | energy threshold $p_D$ | $D=\int(Y/r)^s\,dp$ — **smooth**, energy-driven |
>
> ASDSteel1D's `-fracture` is a **necking/ductile-fracture** law keyed to a uniform-elongation
> threshold; Lemaitre is a **continuum-damage** law keyed to the energy release rate. With the
> default uncalibrated pick ($p_D{=}0.03,r{=}0.12$) the Lemaitre coupon damages *early* and
> peaks low (≈499 vs ASDSteel1D's 545), so the curves look quite different. A **rough
> calibration** — set $p_D=e_{upl}=e_u-s_y/E\;({=}0.098)$ to share the **onset**, then bisect
> $r$ to match the **dissipated fracture energy** $\int\sigma\,d\varepsilon$ — recovers the peak
> (**544 vs 545**) and the energy (**63.8 vs 63.8**) almost exactly. The **only** residual is the
> softening *shape*: Lemaitre's smooth energy-driven decay vs ASDSteel1D's sharper necking drop —
> an irreducible consequence of the two functional forms, *not* a bug. (D2 right panel now shows
> uncalibrated **and** calibrated overlays.)
...
mode shows the same ductile-rupture signature and IMPL-EX robustness, and a two-parameter
calibration ($p_D,r$) matches ASDSteel1D's fracture onset and energy — leaving only the
(by-design) softening-shape difference of the two damage laws.
...
| **E2** | LCF accumulation at $\pm0.05$: $D$ grows monotonically each cycle, increment ~steady in the stabilised tail | monotone; tail within 40%* (top-middle) |
| **E2** | life vs amplitude (cycles to $D{=}0.5$): $\varepsilon_a=0.04,0.05,0.06,0.08,0.10$ | $N=61,30,20,11,8$ — monotone trend (top-right; **gate asserts only $N$ decreases with amplitude**, not the slope) |
| **E3a** | ASDSteel1D cyclic cross-exam: matched backstresses, **undamaged** loops | loops **coincide** — overlay residual $5.8\%\!\to\!1.5\%$ under refinement (bottom-left) |
| **E3b** | **damaged** cyclic loops degrade with monotone $D$ for both models | both soften; $D$ monotone; calibration narrows the gap (bottom-middle) |
...
> ### Reading the cyclic figure — which panel is the cross-code check (and which isn't)
> The E figure's bottom row now separates the two comparisons that an earlier version conflated:
> - **E3a (bottom-left) — UNDAMAGED loops, the apples-to-apples cross-code check.** With matched
>   Chaboche backstresses and damage **off**, the `LadrunoUniaxialJ2` and ASDSteel1D hysteresis
>   loops **coincide** (max residual ≈3.8% on the coarse history, shrinking under refinement).
>   This is the meaningful quantitative agreement: the kinematic-hardening hysteresis is faithful.
> - **E3b (bottom-middle) — DAMAGED loops, apples-to-oranges by construction.** With damage on,
>   the two **different damage laws** (§5) produce different loop degradation — Lemaitre's smooth
>   stiffness+strength fade vs ASDSteel1D's necking-driven collapse. The earlier figure plotted
>   *only* this panel, which could read as "the models disagree." They disagree exactly as much as
>   the two damage formulations do; the calibrated Lemaitre ($p_D{=}e_{upl},\;r$ energy-matched,
>   §5) tracks ASDSteel1D's first-cycle peak markedly better (≈521 vs 532, vs the uncalibrated
>   499). The bottom-right panel annotates the cause.
...
and its **undamaged** kinematic hysteresis tracks the independent ASDSteel1D loops to a few
percent with convergence (E3a, the quantitative cross-code leg). The **damaged** cyclic loops
(E3b) differ by the same margin the two damage laws differ; a rough $(p_D,r)$ calibration to
ASDSteel1D's fracture onset+energy narrows the gap, leaving only the softening-shape difference.
Quantitative LCF-law validation (Coffin-Manson exponent) is not yet gated.
...
- ✅ **Test files committed** — all five `tests/test_lemaitre_*.py` are tracked and run in CI
  (A/B/C/D/E). *(Still pending: **rebuild** to land the `nElem/nNode` profiler-header patch.)*
- ✅ **ASDSteel1D hysteresis cross-check (2026-06-03):** undamaged cyclic overlay (E3a, coincide)
  + Lemaitre↔`-fracture` onset/energy calibration (D2/E3b), gated by
  `test_D2b_calibration_matches_onset_and_energy`. The damaged softening-*shape* difference is a
  documented law difference (§5), not an open defect.
...
```
(Bootstrap the built `opensees.pyd` via `os.add_dll_directory(DIST)+sys.path.insert`. **29
Zone-A cells** — damage 11, validation 9 (incl. A3 rate oracle + B3 ν-independent), cross-exam
4 (incl. D2b calibration), cyclic 5 — **plus 2 Zone-B notched-bar cells** (objectivity +
