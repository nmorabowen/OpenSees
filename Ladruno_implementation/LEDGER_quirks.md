...
- **General form:** before believing any `setParameter`/`updateParameter` flag took effect, read the material's `updateParameter` and check WHICH field of `Information` it reads. `theInt` on a flag reachable only from an interpreter is always a no-op.
