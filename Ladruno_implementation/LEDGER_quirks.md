...
- **Blast radius, MEASURED (ADR-86 PR-2):** only `ModifiedEuler` actually moves results — committed stress by 2.8e-6 relative on a normal-confinement deck, and the `M^b` departure at `p0 = 1.01 kPa` from +18.1297 % to +18.1258 %. **`RungeKutta4`/`RungeKutta45` do not move at all**, because their loops assign `NextVoidRatio` but then read a *separate* `CurVoidRatio` in every `GetStateDependent`/`GetElasticModuli` call (10 uses vs 5 in RK4) — there the bad interpolant only corrupted the RETURNED void ratio. A defect can be real, present at three sites, and load-bearing at exactly one.
