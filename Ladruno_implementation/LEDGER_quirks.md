...
### Vanilla ManzariDafalias initialises `α_in` at the stage flip by ROUND-OFF SIGN (ADR-92 P2-7)
- **Bites:** the elastic stage sets `α = dev(σ)/p` every step (`ManzariDafalias.cpp:1055`), so at the flip from `updateMaterialStage 0` to `1` the material already holds `α = r_gravity ≠ 0` while `α_in = 0`. `Elastic2Plastic()` never touches `α_in` — the init lines that would set it are commented out (`:5139-5141`). The only place `α_in` ever gets initialised is the sign test inside `integrate()` (`:1005-1013`), which fires on the FIRST plastic evaluation after the flip; on a zero-increment step right there (a re-equilibration step, or a hold placed at the flip) the sign of that dot product is round-off noise, not a real loading direction. Vanilla happened to initialise `α_in := α` at roughly half of the points by chance from that noise; the P2-5/5b/5c reversal-noise guards (built to fix the unrelated hold-corruption defect) turned "sometimes" into "never" — they now suppress the reset unconditionally, so `α_in` stays at its uninitialised 0 and `(α − α_in):n` starts large with `Kp` correspondingly small, softening the whole implicit path 23-33 % from step 1 (Esmeralda 146574/146586: first rows 1.296 / 2.6 kN vs 6.511 on `c162833ed`).
- **Consequence:** any zero-increment step placed right after `updateMaterialStage 1` — a re-equilibration step, a hold — silently decides `α_in` per Gauss point by round-off noise. Two decks that are identical except for whether that first post-flip step happens to land exactly on zero increment can diverge 20-30 % in early stiffness, with no warning and no flag touched.
- **Rule:** do not rely on the sign-test side effect to initialise `α_in` at a stage flip. The fork's `-flipAlphaIn init|vanilla` (default `init`) sets `α_in := α` deterministically at every point at the flip, on both the implicit and IMPL-EX paths; `vanilla` is kept only to reproduce the old noise-dependent number for A/B comparison. The reversal-noise guard itself is unchanged — it now applies only to PRIMED states (points that have taken a plastic commit since the flip), so it can no longer intercept the flip's own initialisation.
