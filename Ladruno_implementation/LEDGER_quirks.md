...
- **Workaround/status (2026-08-04):** ⚠ **source-level inference, NOT yet reproduced.** Do not reach for `constraints Auto` as a workaround for the `sp` × plasticity conditioning quirk above. Needs a minimal reproducer (one brick, prescribed face displacement, `Auto` vs `Transformation`), then a 3-line fix plus a regression test in the style of `tests/test_sp_subtract_init.py`; candidate for upstream. See [[80_sp_prescribed_displacement_findings]] §4 candidate B.
