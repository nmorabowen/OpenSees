...
- **Still open (the other half of the same tracked item):** the profiler's HDF5 **run attributes** record `threads` and `nElem` wrong — `Profiler.cpp` sets `m.threads = threads_.size()`, i.e. profiler-*registered* threads, which is 1 on a serial run no matter what `MKL_NUM_THREADS` is. Do not read thread count off a profile; record it from the environment. *2026-07-27.*
