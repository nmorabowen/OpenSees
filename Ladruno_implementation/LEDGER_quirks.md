...
### honest-p LadrunoUP + upstream `CentralDifference` = Richardson-unstable pore pressure (leapfrogged diffusion) — use the ADR-73 overlay or an implicit integrator for the fluid
- **Bites:** running the monolithic honest-p `LadrunoUP` element under upstream explicit `CentralDifference` looks legal (it assembles) but the p-field rides an explicit central-difference update of a DIFFUSION operator — the classic Richardson instability: p oscillates and diverges at any usable Δt.
- **Why:** upstream *UP elements dodge this only via the rate-DOF trick (their "p" DOF is ∫p·dt with S in getMass, factored coupled solve); honest-p keeps H in stiffness, so explicit leapfrog on the p rows is unconditionally unstable for the parabolic part (ADR-73 §7 P3 row; ADR-71 quirk queue).
- **Workaround/status (2026-07-14, row landed with ADR-73 P1 per plan):** fluid explicitness belongs to the OVERLAY lanes (implicit-fluid-at-commit, or P3b lumped-S* forward step at the undrained CFL per the E7.4 pins). The loud symptom-pinning test is owed by ADR-73 P3 (needs the explicit lane in-tree).
...
### UPSTREAM: transient DB restart with ANY LoadPattern in the stream is NON-DETERMINISTICALLY corrupt — deformable `database File` save/restore cannot be trusted
- **Bites:** `database File` + `save`/`wipe`/`restore` of a TRANSIENT model that contains any load pattern does not reproduce the continuous run, and the divergence is **non-deterministic across fresh processes**. Minimal overlay-free reproducer (2026-07-14): body-force-driven elastic quads + ONE ordinary `Plain` pattern with a single nodal load — 3/8 fresh-process restarts diverge (rel 5e-3…3e-2); delete the pattern and the same restart is bit-exact 8/8. With the ADR-73 overlay (large +Q·p forces) the corruption amplifies to O(1)–O(5). In bad runs the restored operator itself is corrupt: generalized eigenvalues of the restored pencil went NEGATIVE with identical analysis setups. Meanwhile every python-visible restored quantity checks out (per-node committed d/v/a, SP sets, pattern state) — the corruption lives below the query APIs, varies with heap/ASLR state, and sometimes vanishes entirely (restart bit-exact), which makes it look like *your* serialization bug. It is not.
- **Why:** unlocalized upstream FileDatastore/Domain-restore defect tied to load patterns in the DB stream (candidates: same-size (dbTag, commitTag) slot clobbers between components, uninitialized reads in a pattern-adjacent recvSelf). Related known holes found en route: FileDatastore does NOT restore a sibling `Plain` pattern's NODAL loads at all; `ops.logFile()` is the only way to capture opserr from python.
- **Workaround/status (2026-07-14, ADR-73 P1):** deformable transient DB restart = **documented UNSUPPORTED** (fork-wide, not overlay-specific) pending the upstream-fix decision (ask-first per the change-budget). Restart gates must isolate: frozen-skeleton round-trips are bit-exact and gate component serialization correctly (the overlay's p/dp gate does this). The overlay additionally hardened two REAL fork-side issues found while chasing this: the FileDatastore same-size clobber (dbTag2 idiom — see the dedicated row) and restore-ordering hygiene (`uSnapshot_` lazily re-derived at first use, never at recvSelf/setDomain time). Pinned by `tests/test_ladruno_overlay_framework.py` 6c (xfail; XPASS = upstream fixed, promote to hard gate).
