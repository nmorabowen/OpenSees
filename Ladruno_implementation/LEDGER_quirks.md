...
### `algorithm Newton -initial` re-assembles AND re-factorizes the initial tangent on EVERY iteration — `ModifiedNewton -initial` is the same algorithm at 1/2 the cost per step
...
- **Workaround/status (2026-07-25, ADR-76 from a TIMs project issue report):** use **`algorithm ModifiedNewton -initial`** (one assembly + one factorization per step), or **`-initial -factoronce`** for one per *analysis* — that pair only started working in this fork's `OPS_ModifiedNewton` as of ADR-76 R4 (see next quirk). The engine-side fix (a tangent-version counter that lets any algorithm skip an unchanged `formTangent`) is designed but NOT implemented; see [[76_ladruno_tangent_reuse_adr]] §4, whose gate is the `getInitialStiff()` audit above.
...
- **Workaround/status (2026-07-25, spun out of the ADR-76 `factorOnce`/`domainChanged` audit):** **FIXED** — `return -info;` in all three (always negative in that branch, and it keeps the failing pivot index in the magnitude; the idiom already used at `SymBandEigenSolver.cpp:228` / `FullGenEigenSolver.cpp:188`). No caller reads the magnitude — every call site gates on `< 0`. Regression deck: `Ladruno_implementation/lapack_singular_regression/`. **Expect fallout:** models that previously limped past a singular step now stop. That is correct, but it will surface latent singularities (free nodes under `constraints Plain`, zero-stiffness materials, fully-released members) that were being masked. See [[LEDGER_vanilla_files]].
