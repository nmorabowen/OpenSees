...
- **Detector (the failure is silent, so it needed one):** `ladrunoDR stabilityMargin` returns `(ω_max·dt/2)²` for the mass actually in use, measured against the tangent at the last M* rebuild — `max_i (dt²/4)Σ_j|K_ij| / M*_i`. It is exactly `f²` while the tangent is unchanged and climbs **past 1** once the model has stiffened past the mass (the deep-plastic mechanism: M* built from a softened tangent, elements then unload elastically). Free — the row-sums are recomputed at every auto-refresh anyway. The integrator prints a one-time WARNING when it crosses. Two diagnostics, deliberately: the runtime margin catches tangent drift, and a parse-time warning catches `-massSafety 1`, which sits at exactly 1.0 forever and so cannot be caught by a `> 1` test.
- **Consumers audited (#728):** `Ladruno_scripts/robust_drive.py` rung 5 now exposes `dr_mass_safety` (default = the integrator's 0.5), logs `stabilityMargin` next to the `dr_settled` verdict, and had `dr_max_steps` recalibrated 4000 → **16000** for the 4× step cost; if a caller pins `max_substeps` low, that global budget is now the binding constraint on a rung-5 excursion. `Ladruno_scripts/robust_solve_tests/torture_dynamics.py` deliberately keeps the default and documents why (measured **250** substeps against its 3000/8000 budgets — 12× headroom — and a single softening truss cannot reach the deep-plastic case).
...
### `LadrunoDynamicRelaxation -mass gershgorin` marches EXACTLY ON the central-difference stability boundary, so a long DR run amplifies round-off instead of damping it — and the integrator exposes no safety factor
- **Bites:** you point DR at a model that is already in exact static equilibrium and hold it there. For the first few hundred steps nothing happens — residual 1e-13 kN, reaction printing to eight figures. Then it walks away from the equilibrium and never comes back. Measured (note 83 §3, Prandtl strip, `LadrunoBrick -formulation bbar`, h0 = 1.0, zero-push hold): the residual grew from **2.6e-13 kN to 8.7e+01 kN over 5000 steps** on a 300 kN problem (~1.006 per step), and the footing reaction drifted from a stationary `15.000000` down through 10–13 kN as the spurious oscillation drove Gauss points past yield. Nothing reports an error; the analysis "succeeds" every step, and on an elastic-plastic model the damage is **permanent** — the state is left genuinely softened, so the run is silently wrong rather than obviously broken. A short DR excursion (a few hundred steps) will not show it at all.
- **Why:** `buildGershgorinDiagonal` (`SRC/analysis/integrator/LadrunoFictitiousMass.h`) sets `m_i = (dt²/4)·Σ_j|K_ij|`. By Gershgorin that bounds `λ_max(M*⁻¹K) ≤ 4/dt²`, i.e. `ω_max·dt ≤ 2` — the central-difference stability limit **with equality, not with margin**. For an FE stiffness the bound is very nearly attained by the highest mode, so the march sits on the boundary, where the amplification matrix is defective and round-off is amplified. The scale-free property that makes the mass parameter-free is exactly what removes the margin.
- **`-dt` is NOT the lever, and this surprises people.** With `-mass gershgorin` the mass carries `dt²`, so the update `du = dt²·a = 4R/Σ_j|K_ij|` has `dt` cancel **exactly**, and `KE ∝ du²` is dt-free too. Measured: runs at `dt = 1.0` and `dt = 0.1` produced **bit-identical** settlement, reaction, q and step counts. Only the RATIO of the analyze `dt` to the integrator's `-dt` matters.
- **Workaround (script-side, no core change):** size the mass for `-dt D` but march at `analyze(n, f·D)` with `f < 1`. That gives `ω_max·dt ≤ 2f` and scales the per-step relaxation by `f²`. **The safe `f` is element-dependent and must be measured, not assumed:** `LadrunoBrick -formulation bbar` fails at f = 0.85 and holds at 0.75; `LadrunoBrick20 -formulation uri` fails at 0.75 and holds at 0.50. `f = 0.5` cleared both, at 4× the step count of f = 1.
- **Reach beyond the note:** `Ladruno_scripts/robust_drive.py` rung 5 and `Ladruno_scripts/robust_solve_tests/torture_dynamics.py` both call `integrator("LadrunoDynamicRelaxation")` with the default `-dt` and analyze at the same dt, i.e. at f = 1. Short excursions are fine; long relaxations are not.
- **Status:** OBSERVATION, not fixed. The clean fix is an integrator-side safety factor (a `-safety $f` folded into the mass prefactor, defaulting below 1), which would also make the knob discoverable instead of requiring the caller to know that the analyze dt and `-dt` are separate. Not opened here — note 83 is a measurement and ships no code. *Learned 2026-08-11 (note 83 §3).*
- **Diagnostic that catches it in one move:** re-impose the model's CURRENT displaced state as a constraint and relax for a few thousand steps. It is already in equilibrium, so any drift is the integrator. Note 83 calls this control DR-0.
