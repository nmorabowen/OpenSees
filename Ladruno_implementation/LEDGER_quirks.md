...
- **Workaround/status:** use openseespy for decks needing fork nD materials, or add the material to the Tcl ladder (each is a ~6-line `else if` calling the existing `OPS_*` factory). NOT fixed — the surface is large and no deck has demanded it yet. *2026-07-26 (ADR-75b G-L3).*
