...
### UW `DruckerPrager` is a TWO-SURFACE model, its tension cutoff sits at `I1 ~ 0` for a small apex regulariser, and its cutoff multiplier is structurally ZERO
...
- **Second bite, worse:** the multisurface residual assembly reads `if (Jact(i) == 1) {R(0)...} else if (Jact(i) == 2) {R(1)...}`, but `Jact` only ever holds **0 or 1** — the `== 2` arm is dead code. So `R(1)` is never assembled, `g` stays lower-triangular with `g(1,1) = 1`, and `gamma(1)` (the cutoff multiplier) is **exactly zero on every branch, corner included**. With `rho_bar = 0` the update `I1 -= 9K*rho_bar*gamma(0) + 9K*gamma(1)` is then identically zero: the cutoff **selects a different tangent but returns no stress**, so `I1` stays wherever the trial state left it, arbitrarily far above `T`. Measured on a single `LadrunoBrick` at uniform triaxial tension: branch 3, `gamma1 = 0.0`, `I1 = 15.000` against `T = 1.099`. Pinned as a sentinel by `tests/test_adr95_dp_branch_response.py::test_tension_cutoff_multiplier_is_structurally_zero_in_vanilla` — if that test ever fails someone has repaired the cutoff, and any ADR-95 conclusion resting on H1 has to be re-derived.
