...
- **Capturing `opserr` in a pytest needs a SECOND `logFile` to flush.** `ops.logFile(path,"-noEcho")` does `opserr.setFile(path,...)` (a buffered FileStream); `ops.wipe()` does NOT flush/close it, so reading `path` right after sees an EMPTY file. Redirect to a throwaway (`ops.logFile(path+".flush","-noEcho")`) first — `setFile` closes (flushes) the previous file — THEN read `path`. (Used by the P6 staleness tests to assert the NOTE fired.)
