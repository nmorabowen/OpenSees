...
### `ManzariDafalias` runs a different soil in parallel/after-restore than in serial -- the wire format has no slot for `m_Presidual`
- **Bites:** any `OpenSeesMP` parallel run or database (`FileDatastore`/`recvSelf`) restore of a `ManzariDafalias` (or `ManzariDafalias3D`/`PlaneStrain`) material at low confinement. The worker/restored copy silently runs with the "residual pressure" apparent-cohesion constant `m_Presidual` at **0** while the serial process beside it runs at **1.01 kPa** (`P_atm = 101`) -- a different soil, same deck, same tag, no error, no warning. Below ~10 kPa confinement this is not a rounding difference: the ADR-86 audit measured the model's own bounding-surface identity `η_end = M^b(ψ_end)` departing **+20 %** at p₀ = 1 kPa when `p_r` is present vs. absent.
- **Why:** `sendSelf` serialises `static Vector data(97)` with `m_Pmin` at `data(96)` (`ManzariDafalias.cpp:636`) and **no slot at all for `m_Presidual`**. On the broker path (`FEM_ObjectBrokerAllClasses::getNewNDMaterial`), the class-tag/null constructor runs first -- it sets `m_P_atm = 0.0` and then calls `initialize()` (`ManzariDafalias.cpp:835-836`: `m_Pmin = 1.0e-4*m_P_atm; m_Presidual = 1.0e-2*m_P_atm;`), so at that moment `m_Presidual = 1.0e-2 * 0.0 = 0.0`. `recvSelf` then restores the real `m_P_atm` from `data(9)` (`:674`) but **never re-runs `initialize()`**, and there is no slot to restore `m_Presidual` from even if it did. `m_Presidual` is read at ~30 mean-stress evaluation sites throughout the integrator (`p = one3*GetTrace(stress) + m_Presidual`), including the one feeding `GetPSI()`, so the drift reaches the state parameter ψ and through it `M^b`, `M^d`, and dilatancy -- not just the stress return.
- **Workaround/status (2026-08-26, ADR-86 PR-1 -- fixed for the LadrunoSANISAND subclass only, vanilla left as-is):** `LadrunoSANISAND` overrides `sendSelf`/`recvSelf` to append one extra `Vector(4)` after the unchanged base `Vector(97)`, carrying `mPresidualInput`/`mPminInput`/`mHonorTolR` across the wire, so a `LadrunoSANISAND` worker or DB restore runs the SAME `p_r` as the serial process. Vanilla `ManzariDafalias` itself is deliberately not touched (D3) -- any deck still declaring `nDMaterial ManzariDafalias` (not the Ladruno subclass) carries this defect unchanged; any MP-vs-serial or restore-vs-live comparison on that class at low confinement is invalid. See [[86_ladruno_sanisand_adr]] §3, [[LEDGER_implementations]] LadrunoSANISAND row.

### `ManzariDafalias::revertToStart()` silently restores the hardcoded low-stress constants mid-analysis
- **Bites:** any driver that calls `revertToStart()` (a manual `ops.reset()`/`wipeAnalysis`+resume idiom, or a framework that resets between load cases) on a `ManzariDafalias`-family material OUTSIDE an `ops_InitialStateAnalysis` block -- which is the common case; the guard exists specifically FOR the initial-state-analysis window, not for an ordinary mid-run reset. The material's low-stress constants silently snap back to the hardcoded vanilla values (`m_Pmin = 1.0e-4*m_P_atm`, `m_Presidual = 1.0e-2*m_P_atm`) at the moment of the reset, even if a caller (or a subclass) had deliberately set them to something else at construction.
- **Why:** `ManzariDafalias::revertToStart()` (`ManzariDafalias.cpp:465-476`) is: `if (ops_InitialStateAnalysis) { /* do nothing */ } else { this->initialize(); }` -- outside the initial-state window it calls `initialize()` **unconditionally**, and by the time any mid-analysis `revertToStart()` fires, `m_P_atm` already holds its real value, so `initialize()` recomputes the two constants from it and overwrites whatever was there. This is exactly the same defect surface as the sendSelf/recvSelf quirk above (same root cause -- every un-audited `initialize()` call site is a place the hardcoded constants can reappear), but reached through a completely different code path (analysis-driven reset, not serialization), so fixing the wire format alone does not fix this one.
- **Workaround/status (2026-08-26, ADR-86 PR-1 -- fixed for the LadrunoSANISAND subclass only):** `LadrunoSANISAND::revertToStart()` replicates the base's `ops_InitialStateAnalysis` guard verbatim but calls ITS OWN `initialize()` shadow (base `initialize()` followed by `applyLadrunoConstants()`), so a mid-analysis reset restores `p_residual`/`p_min` to what the deck asked for, not to 1.01 kPa. This is why a `LadrunoSANISAND` subclass MUST override `revertToStart` and cannot get away with merely shadowing `initialize()` -- shadowing alone would still lose to the base's own `revertToStart` calling the base's own `initialize()` internally via static binding (see the `virtual` quirk below for why `initialize()` cannot simply be made virtual instead). Vanilla `ManzariDafalias` is unchanged -- any deck using it directly still has this trap. See [[86_ladruno_sanisand_adr]] §4.4, §8 risk #1.

### Adding `virtual` to `ManzariDafalias::initialize()` or `GetElasticModuli` would silently change every existing Ramberg-Osgood (`ManzariDafaliasRO`) deck's results
- **Bites:** anyone tempted to "clean up" the two quirks above by promoting `ManzariDafalias::initialize()` (or either `GetElasticModuli` overload) to `virtual` so a subclass can override it the normal way, instead of shadowing + re-applying. It looks like the more idiomatic C++ fix. It is a trap: doing so changes behaviour for a class that was never touched and never mentioned in the change.
- **Why:** `ManzariDafaliasRO` (`SRC/material/nD/UWmaterials/ManzariDafaliasRO.h:93-97`) already declares `void initialize();` and two `GetElasticModuli(...)` overloads with **identical signatures** to the base -- today these are non-virtual **shadows** (name hiding), so a `ManzariDafalias*` pointer to an RO object still calls the BASE version, and only code that has the concrete `ManzariDafaliasRO` type gets the RO version. The instant either base method becomes `virtual`, every one of those identical-signature shadows becomes an **override** instead, and the base integrators -- which hold `ManzariDafalias*`/`this` internally, not the derived type -- start dispatching to the Ramberg-Osgood elasticity in **every existing RO deck**, silently changing its results. A change intended to be inert for RO users would not be inert.
- **Workaround/status (2026-08-26, ADR-86 PR-1 -- avoided, not fixed, because there is nothing to fix):** `LadrunoSANISAND::initialize()` stays a **non-virtual shadow** with the identical signature, exactly like `ManzariDafaliasRO`'s -- it deliberately does NOT add `virtual` anywhere in the base. The subclass instead relies on controlling every method that IS already virtual through `NDMaterial`/`MovableObject` (`revertToStart`, `sendSelf`, `recvSelf`, both `getCopy` forms, `Print`) to "win the last write" on the two protected data members, and calls its own `initialize()`/`applyLadrunoConstants()` explicitly from each of those rather than depending on virtual dispatch into `initialize()` itself. `mElastFlag` is a separate, unrelated `static` trap already recorded elsewhere in this ledger (search "mElastFlag") -- both are reasons the Manzari family resists the naive subclassing idiom. See [[86_ladruno_sanisand_adr]] §4.1 ("the RO hazard").
...
### `ManzariDafalias` IntScheme 1 (ModifiedEuler) IGNORES the `TolR` you passed — its substep error tolerance is hardcoded at `1e-4`
- **Bites:** every deck that tightens the optional 5th argument `TolR` on `nDMaterial ManzariDafalias ... 1 $TanType $JacoType $TolF $TolR` (or `ManzariDafalias3D`/`PlaneStrain`/`RO`/`LadrunoSANISAND`) expecting more accurate substepping. IntScheme 1 is the scheme this fork's own constructor warning *recommends* (over the uncontrolled schemes 3 and 5), so it is the scheme a careful user lands on — and it is the one that throws the request away. Nothing warns; the run converges and reports success at whatever accuracy `1e-4` buys. The tell is that halving `TolR` changes **nothing at all** in the output on scheme 1, while it does change scheme 45.
- **Why:** `ManzariDafalias::ModifiedEuler()` opens with `double T = 0.0, dT = 1.0, dT_min = 1e-6 , TolE = 1e-4;` — a bare literal. `TolE` is what the adaptive substep loop actually tests (`if (curStepError > TolE)`, and the step-size update `q = fmax(0.8*sqrt(TolE/curStepError), 0.1)`), so `mTolR` — which *is* stored, from the parser and from `data(20)` on the wire (`:618`, `:700`) — is simply never read by this integrator. It is not a family-wide convention: the sibling `RungeKutta45()` in the SAME file uses `double T = 0.0, dT = 1.0, dT_min = 1e-3 , TolE = mTolR;`, and `SAniSandMS.cpp:1484` does the same. `mTolR` *is* read elsewhere in `ManzariDafalias` — it is the Newton residual tolerance for the implicit schemes — so grepping for `mTolR` and finding hits does not tell you IntScheme 1 honours it.
- **Workaround/status (2026-08-26, ADR-86 PR-2 commit 1 — seam opened in vanilla, deliberately not wired):** the literal is now `TolE = mHonorTolRInME ? mTolR : 1e-4`, with `bool mHonorTolRInME` a protected base member initialised `false` in all four `ManzariDafalias` constructors, so **vanilla is bit-identical** (a false condition selects the `1e-4` operand verbatim — no arithmetic). A derived class opts in from its own constructor; `LadrunoSANISAND`'s `-honorTolR` deck flag is what will do so. Until a deck sets it, IntScheme 1 still runs at `1e-4` and the quirk above is live for every existing deck. **Do not "fix" this by making the integrators `virtual`** — see the `ManzariDafaliasRO` shadow quirk above; the flag seam exists precisely because `ModifiedEuler()` is non-virtual and binds statically inside the base TU.
- **Note on the two similar names:** `mHonorTolRInME` (base, `bool`, the seam) and `LadrunoSANISAND::mHonorTolR` (subclass, `int`, the deck-level request) are different members on purpose. Grepping for `mHonorTolR` hits both; the `InME` suffix means "in ModifiedEuler".
- **Learned:** 2026-08-26, ADR-86 PR-2 (audit of `ManzariDafalias` against `SAniSandMS`). See [[86_ladruno_sanisand_adr]] §4.1, D7.

### `ManzariDafalias::debugFlag` is a `static const bool` set to `false` — all 61 of its `if (debugFlag)` diagnostics are dead code in every shipped build
- **Bites:** anyone reading `ManzariDafalias.cpp` and concluding the model is well-instrumented because the integrators are dense with `if (debugFlag) opserr << "..."` — singular-matrix reports, "stress state outside the yield surface!", "dGamma cannot be negative!", "Didn't find alpha!", and (the sharp one) the low-`p` stress-clamp notices. **None of them can ever print.** There is no runtime switch, no `-debug` flag, no `setParameter` hook: `const bool ManzariDafalias::debugFlag = false;` (`ManzariDafalias.cpp:56`, declared `static const bool` at `ManzariDafalias.h:219`), so flipping it is a source edit plus a full rebuild of every ManzariDafalias TU. The practical consequence is that when this material does something silently wrong, the code *looks* like it would have told you, and it never did.
- **Why it matters most at the clamp:** two integrator entry guards do not merely report — they **change the answer**. `ModifiedEuler()` on `p < m_Pmin + m_Presidual` and `RungeKutta45()` on `p < m_Pmin` both rebuild the returned stress as `GetDevPart(NextStress) + m_Pmin * mI1`, i.e. they preserve the deviator and *paste in* the floor pressure. Downstream the point reports a stress, converges, and is indistinguishable from a real result; the mobilised strength it carries is whatever `m_Pmin` bought. This is the observable ADR-86 §8 risk 6 and the "`-Pmin` has no behavioural gate" hole both needed.
- **Workaround/status (2026-08-26, ADR-86 PR-2 commit 2 — fixed at the two clamp sites, in vanilla):** both clamps now emit an **ungated** `opserr` naming the material tag, the offending `p` and the threshold. The rest of the `debugFlag` diagnostics are left alone. **The throttle is the whole design problem, and "once per material instance" is the wrong answer:** the clamps sit inside a `while (T < 1.0)` substep loop, inside a Gauss-point update, inside a Newton iteration, inside a load step — *and* every Gauss point is its own material instance, because `getCopy(const char*)` runs a full constructor per integration point (ADR-86 PR-1 measured 513 echo lines from a 64-element mesh, extrapolating to ~400k lines / 83 MB of stderr on a 50k-element model). What is actually bounded is a **process-wide budget**: a function-local `static int`, first 10 events print, then one suppression notice, then silence — 11 lines per site regardless of mesh size. Use this pattern for any new diagnostic anywhere inside a material update; per-instance latching is not a throttle in OpenSees.
- **Learned:** 2026-08-26, ADR-86 PR-2. See [[86_ladruno_sanisand_adr]] §6, §8 risk 6.

### The Manzari-family `D_factor` dilatancy sigmoid is DIMENSIONAL — a bare `7.2713` multiplying a raw stress, so the same deck is a different soil in kPa, Pa and MPa
- **Bites:** any `ManzariDafalias` / `ManzariDafalias3D` / `PlaneStrain` / `RO` / `SAniSandMS` / `LadrunoSANISAND` deck that declares `P_atm` in anything other than **kilopascals**, at low confinement. The guard reads `if (p < 0.05 * m_P_atm) D_factor = 1.0/(1.0 + exp(7.6349 - 7.2713*p));` — the *trigger* scales with the unit system, **the sigmoid inside it does not**. `7.2713` multiplies a raw stress, so it has to carry units of 1/stress, and it was calibrated in kPa. At a true confinement of 1 kPa the dilatancy factor comes out **0.410 in kPa, 1.000 in Pa (the sigmoid never fires), 0.0005 in MPa (dilatancy is annihilated)**. Three different materials from one input file, converging and reporting success in all three. OpenSees has no unit system, so nothing can catch this.
- **Why it is easy to miss:** the surrounding code is *scrupulous* about `m_P_atm` — `GetPSI` uses `(p/m_P_atm)^ksi`, `b0` uses `sqrt(p/m_P_atm)`, the trigger itself is `0.05*m_P_atm`. The one dimensional constant sits in the middle of correctly non-dimensionalised neighbours, which reads as deliberate. It also appears at **four** sites in `ManzariDafalias` — `GetStateDependent` and three byte-identical replicas in the analytical-Jacobian lane (`NewtonSol`, `NewtonSol_negP`, `GetJacobian`), each with **its own derivative** `be*temp1/pow(1+temp1,2)`. Patch the value at some sites and not others, or the value without the derivative, and the consistent Jacobian stops linearising its own residual; the symptom is a convergence defect that gets blamed on the integrator.
- **Fix and the trap inside the fix (2026-08-26, ADR-86 PR-2 commit 3 — fixed in vanilla, all four sites, D5b):** rewrite as `1/(1 + exp(a - b*p/m_P_atm))` with `a = 7.6349` unchanged and `b = 7.2713*101.0`. **Spell the product out in the source; do NOT write the rounded `734.401`** — `734.401/101 = 7.27129703`, a 4.1e-7 relative shift, so the rounded literal is *not* the no-op it looks like, while `(7.2713*101.0)/101.0 == 7.2713` bit-for-bit in IEEE double. Group it left-to-right (`7.2713 * 101.0 / m_P_atm * p`) so the constant folds, divides back exactly, and only then multiplies `p`. In the three Jacobian blocks the local `double be` **is** `d(exponent)/dp`, so redefining `be = 7.2713*101.0/m_P_atm` and routing `temp1`/`D_factor` through it rescales value and slope in one edit. Verified as an exact no-op three ways: the bit-identity above, 500 000 random `p` across the whole trigger domain with zero mismatches in value or derivative, and a 1200-step low-`p` drained triaxial returning bit-identical committed stress hex-for-hex.
- **The sibling copy is worse, and is NOT fixed:** `SAniSandMS.cpp:2627` carries the same bare constants at one site, but its trigger is `p < 0.001*m_P_atm` (0.101 kPa), not `0.05*m_P_atm`. Over that entire window the factor runs 4.8e-4 … 1.0e-3, i.e. it is not a smooth ramp at all but a ~1000x hard kill of dilatancy — constants tuned for a 5.05 kPa window applied in a 0.101 kPa one. `NTUASand02.cpp`, which ADR-86 §7.2 names as the third copy, **is not present in this tree**.
- **Open (ADR-86 D5a):** whether this sigmoid should exist at all once `m_Presidual = 0`. It and the residual pressure look like overlapping patches for the same low-`p` problem applied at different times. Non-dimensionalising the constants does not settle that.
- **Learned:** 2026-08-26, ADR-86 PR-2. See [[86_ladruno_sanisand_adr]] §7.2, §7.2.1, D5a/D5b.

### `ManzariDafalias`'s substep loops interpolate the void ratio FROM THE WRONG END — and `PM4Sand` in the same folder proves it is a typo, not a modelling choice
- **Bites:** inside `ModifiedEuler`, `RungeKutta4` and `RungeKutta45`, the substep pseudo-time `T` sweeps 0 -> 1 across the strain increment, so the void ratio at `T` must be evaluated at `CurStrain + T*(NextStrain - CurStrain)`. All three evaluated at **`NextStrain` + T*(...)** — a full increment ahead at `T=0`, overshooting by one at `T=1`. Void ratio -> psi -> `M^b`, so it displaces the bounding surface.
- **Why it is unambiguous:** `PM4Sand::ModifiedEuler` and `PM4Silt::ModifiedEuler` (same folder, same UW lineage) use the **same** `tmp0` accumulate idiom in the **same** loop and both start from `CurStrain`. In every one of these files the code faithfully implements its own commented-out reference line — and Manzari's comment says `NextStrain` while PM4Sand's says `CurStrain`. So the error is in Manzari's original expression; the later optimisation pass preserved a typo rather than introducing one.
- **Blast radius, MEASURED (ADR-86 PR-2):** only `ModifiedEuler` actually moves results — committed stress by 2.8e-6 relative on a normal-confinement deck, and the `M^b` departure at `p0 = 1.01 kPa` from +18.1297 % to +18.1258 %. **`RungeKutta4` does not move; `RungeKutta45` DOES.** The original claim that both were inert was REFUTED by an adversarial pre-merge review and independently re-verified: RK4's loop reads a separate `CurVoidRatio` (7 in-loop reads, 0 of `NextVoidRatio`), so there the bad interpolant only corrupted the RETURNED void ratio -- but **RK45's loop reads `NextVoidRatio` at all 11 in-loop `GetStateDependent`/`GetElasticModuli` sites and `CurVoidRatio` at none**, so RK45 is a genuine behaviour change. Measured by A/B rebuild on a NON-isochoric drained triaxial: scheme 1 and scheme 3 bit-identical, **scheme 45 reldiff 6.33e-3** (sigma_zz -1204.82 vs -1209.38 kPa). PR-2's own measurement reported zero because the confine-first test deck's deviator leg is **exactly isochoric** (trace(d_eps) = 0) and the two interpolant anchors differ by exactly trace(d_eps) -- true on that deck, falsely generalised. A defect can be real, present at three sites, and load-bearing at exactly one.
- **Status:** fixed at all three sites in one commit, ADR-86 PR-2. `SAniSandMS::RungeKutta4:1550` carries the identical error and is deliberately left for that material's own PR. `NTUASand02` is not in this tree.
- **Learned:** 2026-08-27.
...
### `ManzariDafalias::getState()` is SHADOWED by all four wrappers and reports a different `alpha_in` than they do — investigated for ADR-86 PR-2 and deliberately NOT "fixed"

- **The shape of it.** `getState()` returns a 26-vector `[eps_e(6) | alpha(6) | fabric(6) | alpha_in(6) | e | dGamma]`. `ManzariDafalias::getState` (`ManzariDafalias.cpp:882`) assembles **`mAlpha_in_n`** — the COMMITTED reversal reference — at offset 18. All four vanilla wrappers assemble the TRIAL **`mAlpha_in`** at the same offset, with byte-identical signatures: `ManzariDafalias3D.cpp:153`, `ManzariDafaliasPlaneStrain.cpp:176`, `ManzariDafalias3DRO.cpp:136`, `ManzariDafaliasPlaneStrainRO.cpp:176`. So do our own `LadrunoSANISAND3D.cpp:177` and `LadrunoSANISANDPlaneStrain.cpp:205` (PR-1 copied the wrapper shape). `commitState` (`:433`) does `mAlpha_in_n = mAlpha_in;`, which is what makes `_n` the committed twin.
- **`getState` is NOT virtual**, so which body runs is decided by the STATIC type at the call site. Worth knowing on its own.
- **But the wrapper bodies are unreachable in practice.** `setResponse`/`getResponse` exist only on the base (`:519`, `:546`) and are not shadowed; inside them `this` is a `ManzariDafalias*`, so `this->getState()` binds to the BASE's body **even for a `ManzariDafalias3D` object**. Every recorder therefore gets `mAlpha_in_n`. A grep of `SRC/` finds **no** caller holding a `ManzariDafalias3D*` / `ManzariDafaliasPlaneStrain*` at all, so the four wrapper `getState` bodies are dead code for every path in the tree. The base's version has defined 100 % of shipped behaviour since the class was written.
- **And nothing observable ever distinguished them anyway.** `commitState` copies `mEpsilonE`, `mAlpha`, `mFabric`, `mDGamma` and `mAlpha_in` to their committed twins and recomputes `mVoidRatio`, and `Domain::commit()` records AFTER committing — so at a recorder's actual call moment all six entries are simultaneously the committed state under either spelling. The two forms can only differ if `getState` is called mid-iteration (a script's `eleResponse` after a failed `analyze`).
- **Why ADR-86 PR-2 changed NOTHING here.** The evidence points both ways and no fact in the code settles it:
  - *For the wrappers (trial):* five of the six entries in the vector are trial, so the base's slot 18 makes the snapshot mixed-time; and six sites spell it `mAlpha_in` against the base's one.
  - *For the base (committed):* slot 18 then agrees exactly with the class's own dedicated `getAlpha_in()` accessor (`:911`, response type 6, the `alpha_in` recorder), which also returns `mAlpha_in_n`. Change `getState` and `state[18..23]` starts contradicting the `alpha_in` recorder on the same material. `mAlpha_in` is also a *reversal reference* that can flip within a step, so reporting the committed one is defensible on its own terms.
  - *The family does not settle it either:* in the same folder, `PM4Sand::getState` (`:948`) assembles `mAlpha_n`, `mFabric_n`, `mAlpha_in_n` — all COMMITTED — while `PM4Silt::getState` (`:971`) assembles `mAlpha`, `mFabric`, `mAlpha_in` — all TRIAL. Two sibling materials, same lineage, opposite answers.
  - *And it is not a copy-paste slip that drifted:* the divergence is present in **every** revision back to the 2014 introduction of `ManzariDafaliasRO` (`78d5bcc82`) and has never been touched since. It was written that way.
- **Verdict:** ADR-86 D9's error-vs-opinion test lands on **opinion**. Changing the base is the riskier edit (it is the only reachable one) and changing the wrappers picks a winner with no evidence, so PR-2 reports and leaves it. If the fork ever wants uniformity, the honest fix is to decide the whole vector at once — committed *or* trial for all six entries — not to patch offset 18.
- **What you must not do about it:** `getState` is a sibling of the `initialize()` / `GetElasticModuli` shadows at `ManzariDafaliasRO.h:93-97`. Do **not** "fix" the static-type hazard by adding `virtual`; see the dedicated entry above.
- **Learned:** 2026-08-27.
...
### UW `DruckerPrager` is a TWO-SURFACE model, its tension cutoff sits at `I1 ~ 0` for a small apex regulariser, and until ADR-95 P4 its cutoff and corner branches were DEAD CODE with a wrong tangent
- **Bites:** you calibrate `nDMaterial DruckerPrager` as "a Drucker-Prager cone with a tiny cohesion `SY` to regularise the apex", and reason about the model as if the cone were the only yield surface. It is not. The material carries a second surface — a **tension cutoff** `f2 = I1 - T` with
  `mTo = sqrt(2/3) * sigma_y / rho` (`DruckerPrager.cpp`, full constructor) — and at the ADR-95 / TIMs deck's `SY = 0.2 kPa`, `phi_txc = 20 deg` that puts `T = 1.099 kPa`, i.e. **essentially at zero mean stress**. Every Gauss point that heaves into tension (the ground surface beside a footing edge, the whole free surface of a strip-footing deck) therefore changes CONSTITUTIVE OPERATOR, not just state — `mCep` is assembled from a different `b1`/`b2`/`g_contra` in the `f2` and corner arms. Nothing warns you; the recorder `state` response does not name the branch. Note the cone apex sits at `I1 = T` too, so a point that crosses the cutoff crosses the cone in the same instant: pure tension gives the **corner** (branch 3), never `f2` alone.
- **Second bite, worse:** the multisurface residual assembly reads `if (Jact(i) == 1) {R(0)...} else if (Jact(i) == 2) {R(1)...}`, but `Jact` only ever holds **0 or 1** — the `== 2` arm is dead code. So `R(1)` is never assembled, `g` stays lower-triangular with `g(1,1) = 1`, and `gamma(1)` (the cutoff multiplier) is **exactly zero on every branch, corner included**. With `rho_bar = 0` the update `I1 -= 9K*rho_bar*gamma(0) + 9K*gamma(1)` is then identically zero: the cutoff **selects a different tangent but returns no stress**, so `I1` stays wherever the trial state left it, arbitrarily far above `T`. Measured on a single `LadrunoBrick` at uniform triaxial tension: branch 3, `gamma1 = 0.0`, `I1 = 15.000` against `T = 1.099`. Pinned as a sentinel by `tests/test_adr95_dp_branch_response.py::test_tension_cutoff_multiplier_now_returns_stress` — if that test ever fails someone has repaired the cutoff, and any ADR-95 conclusion resting on H1 has to be re-derived.
- **Third bite:** at *exact* hydrostatic tension with `rho_bar = 0`, `norm_eta` is 0 both before and after the return map, `n` is zeroed, and the tangent's last term `- 4G^2/norm_eta * gamma(0) * (IIdev - n(x)n)` divides by zero — `mCep` comes back non-finite, and `NormCep < 1e-10` does not catch it. Any probe that drives this material to a perfectly hydrostatic tensile state must perturb the deviator.
- **`Jact = ` / `count = ` on stderr means the forced-accept fired.** `plastic_integrator` prints those two lines only from the `count > 3` bailout, which abandons the Kuhn-Tucker search, forces `Jact = (1,1)` and accepts the corner. Their **absence** in a log is evidence the bailout did NOT fire — which is how ADR-95 excluded that path as the trigger for the quadratic wall before instrumenting anything.
- **How to see any of it:** `eleResponse(tag, 'material', gp, 'ladrunoBranch')` (ADR-95 P0, PR #803) returns `[branch, gamma0, gamma1, f1_trial, f2_trial, forcedAccept, I1, detAmin]`; `branch` is `Jact(0) + 2*Jact(1)` read AFTER the whole `while(!okay)` loop, and `detAmin` is `min_n det(n.D_ep.n)/(2G)^3`.
- **Fourth bite, measured at P0 and load-bearing for ADR-95 H2:** with `rho_bar = 0` (fully non-associated) and perfect plasticity (`H = theta = 0`), `detAmin` goes **negative the instant a Gauss point yields on the cone** — a single `LadrunoBrick` under uniform compression + shear reads `detAmin = -7.5e-3` at first yield, against the exact elastic value `(K + 4G/3)/(8G) = 0.4792`. Loss of ellipticity in this material is therefore the GENERIC state of any plastic Gauss point, not a rare event, so "`detAmin < 0` appeared" is worthless as a discriminator; only the **fraction / spatial connectivity** of negative-`detAmin` points can be compared across elements.
- **Date:** 2026-09-07 (ADR-95 P0). Cross-links: `Ladruno_implementation/95_prandtl_reissner_quadratic_root_cause_plan.md` §2 (H1), [[LEDGER_vanilla_files]] rows for `DruckerPrager.{h,cpp}`.
- **FIXED in ADR-95 P4 (PR #803) -- and this is the headline: any non-associated DP leg that reaches `I1 >= T` on a PRE-FIX build dies in Newton with a ~1000x drop in the smallest singular value of the global operator.** The second and third bites above were one defect, and it had a fourth face nobody had looked at: because `g(1,1)` stayed at the dummy `1`, the INVERSE `g_contra` used to build the consistent tangent was dimensionally wrong (`g_contra(1,1) = +1` where it should be `-1/(9K)`), so the corner tangent picked up two spurious rank-one terms in stress-squared units -- `+9K^2 * 1(x)1` and `-27*rho*K^2 * n(x)1` -- scaling as `9K^2/(2G)` and `27*rho*K^2/(2G)` relative to `2G`. That is the 5-8 decade `detAmin` outlier ADR-95 P1 measured at the first four Gauss points to cross the cutoff (`-3e5 ... -6.6e7` against `-0.06` everywhere else), and it enters the GLOBAL operator, which is why the symptom is a global Newton seizure and not a local material warning. P4 makes the assembly index-driven (`if (Jact(0)==1) {row 0} if (Jact(1)==1) {row 1}` -- the row index is the SURFACE index, not the loop counter) and fixes the one genuinely wrong factor in the tangent block: the radial-return softening term must divide by `||eta_TRIAL||`, where upstream divides by the norm of the RETURNED eta that it has just recomputed for `mState(1)`. **Upstream master still carries both** (checked 2026-09-07: dead `Jact(i)==2` arms at lines 495 and 530, the final-norm divide at line 670), as does the fork's own sibling `SRC/material/nD/DruckerPragerThermal.cpp` (lines 751, 787) -- so treat ANY other copy of this return map as defective until checked.
- **Two consequences that look like new bugs but are correct answers.** (a) `mTo` is not a free parameter: the constructor sets `T = sqrt(2/3)*sigma_y/rho`, which is exactly the cone apex, so on a NON-hardening deck `f1 = f2 = 0` forces `||eta|| = 0` -- every corner return lands on the apex, the stress is pinned at `sigma = (T/3)*1`, and the exact consistent tangent is **zero**. The pre-existing `NormCep < 1e-10` floor catches that and hands the solver `1e-3*Ce`; P4 throttles its `NormCep = ...` message to 10 lines because it now fires at every apex GP. (b) The **f2-only branch is algebraically unreachable** without hardening (`f1 <= 0 < f2` would need `||eta|| < 0`), which is why P1 never saw branch 2 and why a test for it needs `theta = 1, H > 3G`.
- **The third bite (hydrostatic NaN) is fixed too**, by guarding the softening coefficient with the same `> 1e-13` test the code already applies to `n`; a probe no longer has to perturb the deviator.
- **Date:** 2026-09-07 (ADR-95 P4, PR #803). Cross-links: `Ladruno_implementation/_adr95_p4_results.md`, `_adr95_corner_tangent_review.md`, `tests/test_adr95_dp_corner_fix.py`.
...
- **What it still does NOT promise.** (1) The flag is OFF by default and off is byte-identical to upstream — a model that has never set it has none of this. (2) The check is `f > f_absolute_tol` with an ABSOLUTE tolerance in stress units, so the unit system still decides whether a state is refused (see the `f_absolute_tol` entry above). (3) `stdBrick` swallows the refusal anyway (`Brick::update()` returns 0 unconditionally); use `LadrunoBrick` or `TenNodeTetrahedron` for anything you want to fail loud. (4) It is an ADMISSIBILITY gate, not an accuracy gate: a state can pass `f <= tol` and still be the wrong point on the surface (ADR-94 M3/B5).
- **Rule:** turn it on for any softening or near-limit deck, and read a refusal as "cut the step", not as "the model is broken". A step that now fails where it used to converge is almost always a state that used to be committed wrong.
- **What flag-OFF actually costs (MEASURED, R4 matrix regenerated on `229842f7f`):** the strict helper is constant-false with the flag off, and the `Forward_Euler`, `Forward_Euler_Subincrement` and `Backward_Euler` rows of the R4 matrix regenerate **completely unchanged** across all 5 tangents and 3 paths. Every difference is confined to three integrators and is item-1/item-5 behaviour, not strict mode: `Backward_Euler_LineSearch` is now `crashed` (the parser refuses it, ADR-94 M7); and `Modified_Euler_Error_Control` / `Runge_Kutta_45_Error_Control` flip `ok` -> `refused` with **identical `|f|max`** because their bare `-1` used to be dropped by `LadrunoBrick`'s sentinel-only compare and is now heard. The ONE genuine numeric change in the whole table is `HoekBrown` x RK45 x compression, `5.55e-09` -> `2.35e-08` at the same iteration count -- HoekBrown uses `NullHardeningTensorFunction`, whose policy used to RETURN uninitialised Eigen storage (ADR-94 B4), so this is the Eigen-init fix showing up as a 1e-8 shift.
- **`compute_local_stress`'s gate is inert by construction:** its three callers are the numerical-tangent probes and all ignore the return code, so the site returns the sentinel for contract uniformity only. Do not use it as evidence that the numerical tangents are gated.
...
### `wipe` zeroes `implexRefusals` / `implexGuards` / `avgImplexError`; `reset()` does not; the `opserr` throttles never do (WP-104, 2026-09-15)
- **Symptom (before WP-104, apeGmsh live test on fork tip `634824e1f`):** a FRESH `nDMaterial LadrunoSANISAND` in a NEW model -- after `ops.wipe()`, with a different tag, before any `analyze()` -- read `implexRefusals = [9,0,0,9,0,9]`: the nine companion refusals an EARLIER model in the same process had latched. Constant for the whole new run (inherited, not accrued). Any end-of-run `implexRefusals[3] == 0` check (the guide's control-off discipline, the apeGmsh guide's `implex_refusals_companion == 0`) therefore passed or failed on PROCESS ORDER.
- **Cause:** slots 0-3 and 5 of `implexRefusals`, all 7 slots of `implexGuards`, and `avgImplexError` (+ its commit-round marker) live in ONE anonymous-namespace singleton, `LadrunoImplexGlobals` in `SRC/material/nD/LadrunoSANISAND.cpp`. Only slot 4 (`commitLatched`) is a member of the instance. Nothing in either interpreter's `wipe` reached the singleton; the 2026-09-07 entry above recorded that as "by intent" for arms of one model, and the apeGmsh test is the first thing that ran two models per process and read the ledger fresh.
