...
- **Workaround/status:** put 2D beam mass on the nodes (`ops.mass`) for ground-motion runs, or use `ElasticBeam3d`/a fork beam. Do NOT use a 2D `elasticBeamColumn` with element `-mass` as a -Q oracle — `tests/test_rayleigh_inertia_bezier_imk.py` uses nodal masses for exactly this reason. Upstream fix (drop the second `P.addVector(1.0, Q, -1.0)` in `getResistingForceIncInertia`) NOT applied: vanilla-footprint rule, raised with the owner (WP-115, 2026-09-23).
