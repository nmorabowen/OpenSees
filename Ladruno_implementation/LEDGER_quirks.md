...
- **CONSISTENT (Olovsson) is the opposite:** `consistentPCG`/`consistentMatVec` (`LadrunoMassScaling.h`) run a matrix-free Jacobi-PCG with **rank-local** inner products (`res ^ z`, `p ^ Ap`) and no shared-DOF `M̄` matvec exchange — NOT parallel-safe. A real distributed CG (MPI_Allreduce'd dot products + shared-DOF `M̄` accumulation) is the open increment; the consistent-variant warnings already say "not reduced".
- **Tcl can't reach SMS at all:** `integrator CentralDifferenceSMS …` errors "No Integrator type exists" in the *Tcl* `OpenSeesMP.exe`/`OpenSees.exe` — SMS is registered only in the interpreter/openseespy layer (`SRC/interpreter/OpenSeesCommands.cpp`), NOT the legacy `SRC/tcl/commands.cpp:5588` `integrator` parser (which knows only `CentralDifference`), even though the Tcl splash banner advertises it. Validate/use via openseespy (`import openseesmp`), or wire the SMS classTags into the legacy Tcl parser (follow-up). **CI caveat: the 2-rank `mpiexec` test is single-process-CI-ungated — local validation only.** Learned 2026-06-21, T-MPI lumped validation, [[36_ladruno_selective_mass_scaling_adr]].
