...
- **Workaround/status:** call first, store the bool, then `require(ok, "... " + message)`. Fixed in `checkRankPermutationInvariance`; the same idiom **still appears elsewhere** in `ladruno_cms_hierarchy_check` (e.g. the `makeAssemblyRecord` calls in `checkDistributedFourRankFlow`) and is worth a mechanical sweep. *2026-07-26 (ADR-1000 P3d).*
