...
- **Workaround/status (2026-07-18, ADR-73 P3):** UNSUPPORTED combination — BOTH `LadrunoMassScaling` builders (lumped `buildMassScaling` AND consistent `buildMassScalingConsistent` — the adversarial verifier caught the consistent path initially silent) print a shared loud one-time warning at sizing when any overlay is present; choose `dtTarget` against the overlay-aware `criticalTimeStep()` report (x the 0.5 ADR-73 margin) instead of the SMS certification. The fix (passing the augmentation through the existing `elementCriticalDt` Kadd seam + the SMS composability gate) is the ADR-73 P3b runway.
