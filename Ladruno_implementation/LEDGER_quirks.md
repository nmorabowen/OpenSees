...
- **Bites:** any model in which every DOF is either `fix`ed or carries an `sp` under `constraints Transformation` — the natural shape of a **single-element prescribed-strain patch test** (unit cube, 1/8-symmetry fixes, remaining faces driven by `sp`), and also of a **staged model** whose last free DOF gets prescribed mid-run. `analyze(1)` does not return an error code: the Python process **dies**, exit 255 / -1, with no traceback, nothing on stderr, and `faulthandler` shows nothing. Under `system UmfPack` the identical model returns rc=0, so it looks material- or element-specific until you A/B the solver. Reproduced identically on `ElasticIsotropic`, `ASDPlasticMaterial3D` MC and MCTC — material-independent. Cost a session about an hour of bisection, most of it spent suspecting the test file.
- **Why:** two separate holes, both in `setSize(Graph&)`.
  (1) `FullGenLinSOE`, `BandGenLinSOE`, `BandSPDLinSOE`, `ProfileSPDLinSOE`, `SProfileSPDLinSOE` all build their `vectX`/`vectB` (FullGen also `matA`) `Vector` wrappers inside `if (size != oldSize)` — and `DiagonalSOE` excludes `size == 0` from that block explicitly (`size != oldSize && size != 0`), same consequence. Zero equations means `size == oldSize == 0`, the block never runs, and the wrappers keep the **null** value the default constructor gave them. `getX()`/`getB()` then take their `if (vectX == 0) { opserr << "FATAL ..."; exit(-1); }` branch. `exit(-1)` is a clean process exit, not a signal — hence no traceback and no faulthandler report — and the FATAL message goes to `opserr`, which the Python module redirects, so nothing is printed either. The solvers were **not** the problem: every LAPACK solver already had its `if (n == 0) return 0;` quick return.
