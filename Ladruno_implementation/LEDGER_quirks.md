...
- **Consistent SMS energy: the `EnergyBalanceRecorder` KE does NOT include the M_bar term (v1 documented gap).** KE is `½vᵀMv` summed over element+nodal `getMass()` (`EnergyBalanceKernel.h`); the consistent scaling mass `M_bar` lives only in the integrator's matrix-free blocks, never in `Node`/element mass, so the recorder under-reports KE by `½vᵀ(ΣM_bar)v` and the RES closure does NOT close for `CentralDifferenceSMSConsistent` (it DOES for the lumped sibling, whose nodal injection is visible to the recorder). The leap-frog itself still conserves the true `M̃`-energy. A correct fix needs a recorder↔integrator KE hook (coupling) — deferred to v2. Small in the design regime (low-participation scaled elements). Learned 2026-06-20, [[38_ladruno_consistent_mass_scaling_adr]] V4.
