...
project: Ladruno
status: implemented (opt-in; default flip pending)
priority: high
owner: nmora
tags:
  - implementation
...
- **2026-09-08 — P5 (`wp/97f-explicit-gate`, PR [#827](https://github.com/nmorabowen/OpenSees/pull/827), build `a1b14d0f1`).** Two independent pieces on the same build, per the plan's P5 row. **D5 gate:** a new `Begin_Integration_Options` token, `experimental_integrator` (int, default 0), gates the four explicit integrators (`Forward_Euler`, `Forward_Euler_Subincrement`, `Modified_Euler_Error_Control`, `Runge_Kutta_45_Error_Control`) behind an opt-in — ADR-94 M2/M8's real defects (empty drift-correction bodies, an error controller that accepts unconditionally at `rk45_dT_min`) are gated, not rewritten (a different ADR); the same treatment ADR-94 gave `Backward_Euler_LineSearch`/`Runge_Kutta_45_Error_Control_old`, one notch softer because these four have real users. The check resolves AFTER the whole option loop, like the existing `Algorithmic`/`Closest_Point` cross-check, so token order does not matter; `Backward_Euler` and `Closest_Point` are untouched. **M9 (StiffSoilShear NaN root cause):** TWO independent bugs, both fixed. (1) `StiffSoilShear_YF.h`'s `qf` used an unguarded `cot(phi)`, `Inf` at `phi == 0` (a legitimate cohesive-only/undrained-clay choice); `Inf * 2*sin(phi)` is the IEEE-754 indeterminate NaN — rewritten algebraically to `qf = 2*(c*cos(phi)+sigma3*sin(phi))/(1-sin(phi))`, numerically identical for `phi != 0` (~1e-14 relative), correct Tresca limit `qf -> 2c` at `phi == 0`. (2) `StiffSoilShear_PF.h`'s numerically-differentiated flow direction normalizes by its own norm with no zero-guard; at an EXACTLY hydrostatic trial stress `computeMobilizedDilatancy()` returns `psi_m == 0` exactly, collapsing the central-difference numerator to the exact zero vector on all six Voigt axes — `norm == 0.0`, `vv_out /= norm` was the indeterminate 0/0. **This IS the ADR-94 R3a "6/194 non-finite cloud points" finding** (the 6 points are exactly the hydrostatic-axis points, not "random general (non-diagonal) points" as originally logged — `_adr94_components.md` corrected). Fixed with a `setZero()` zero-guard, the same precedent the DruckerPrager `NaN*0` fix used. `Closest_Point` continues to refuse the whole StiffSoil family (D3, unchanged); D6's `ELASTICITY_STRESS_DERIVATIVE` for `StiffSoil_EL` is therefore NOT required (recorded, not implemented). **Measured:** gate 4 re-run — the 23-deck / 282-row `Backward_Euler` baseline still byte-identical, INCLUDING the four explicit-integrator decks (`cube/vm/{Forward_Euler,Forward_Euler_Subincrement,Modified_Euler_Error_Control,Runge_Kutta_45_Error_Control}/Continuum`), which now opt in via `experimental_integrator 1` in the gate-4 dumper (D5 is a parse-time gate, not a behavior change — the baseline data itself is untouched). Refusal coverage: all four explicit methods checked BOTH directions (unset/0 refused, 1 accepted), a typo'd option name still rejected, `Backward_Euler`/`Closest_Point` unaffected across all three flag states, `tangent_type Algorithmic` still refused on an opted-in explicit integrator (closing a coverage gap D5 would otherwise open in the pre-existing `test_algorithmic_is_refused_with_any_other_integrator`, three of whose four parametrized methods would otherwise be refused by D5 FIRST — the same "refused, but for the wrong reason" class the P3 report's `HB_sigma_ci` finding warned about), and two child-process real-stdout/stderr assertions for the refusal message text (pytest's `capfd` cannot see this `.pyd`'s `cout`/`opserr` on this build — the ADR-94 `_run_child` idiom). StiffSoilShear: Bug A (`phi == 0`) finite at step 1; Bug B (isotropic leg) finite at step 1, with the deeper path's own separate clean refusal (`rc == -3`, never a NaN commit) noted as a distinct, non-Bug-B limit; a single-leg triaxial-compression hardening path (16 steps) never commits `q` past the hyperbolic law's own asymptote `q_a = qf/Rf` (a hardening-state-independent invariant of the corrected `qf`, avoiding the need to query the internal `EpsQpShear` accumulator directly); a `StiffSoilCap` smoke test (phi in {0,30}, cap IV in {0,100}) all finite; `Closest_Point` refusal for both StiffSoilShear and StiffSoilCap checked in both directions. Full battery (`test_adr84*`, `test_adr94*` except `test_adr94_matrix.py` execution, `test_asdplastic_*`, `test_adr97*`): **230 passed, 2 skipped (pre-existing)**. **Found while implementing:** `test_adr94_matrix.py` has a module-scoped fixture that regenerates its own tracked `_adr94_matrix.md` doc as a side effect of running its one test; `--ignore=test_adr94_matrix.py` (bare filename) inside a glob-expanded `pytest test_adr94*.py` invocation silently failed to exclude it and the doc was overwritten (recovered via `git checkout`, not committed) — the file is edited (`_int_opts()` gains `experimental_integrator 1`, harmless for `Backward_Euler`) but verified ONLY by `pytest --collect-only`, never executed, and any future exclusion must build an explicit file list rather than trust a bare `--ignore` filename against a glob. All in [[LEDGER_quirks]]. Full report: [[reviews/adr97_p5_report]].
- **2026-09-07 — P4 (`wp/97e-numalg-repoint`, PR [#829](https://github.com/nmorabowen/OpenSees/pull/829), build `7e93e4381`).** `tangent_type Numerical_Algorithmic_FirstOrder/SecondOrder` re-pointed at the ACTUAL committed map (`Backward_Euler` or `Closest_Point`, whichever `integration_method` is configured), closing ADR-84 P4 / ADR-94 M3's "third map" finding: both options used to differentiate `compute_local_stress()`, a simplified single-shot elastic-predictor / one-step plastic-corrector neither real integrator ever calls (measured 31% / 4.6% off the true consistent tangent). New private member `bool suppress_numerical_tangent = false;` (per-instance recursion guard — `Backward_Euler`/`Closest_Point` both call `ComputeTangentStiffness()` at the tail of every successful `setTrialStrainIncr()`, so an unguarded perturbed sub-call would try to compute another numerical tangent of its own perturbed state, unbounded) and a new private `numerical_tangent_of_committed_map()` that snapshots every member the real integrators mutate (`TrialStrain`/`TrialStress`/`TrialPlastic_Strain`/`iv_storage`/`cp_last_iterations`/the four `mutable` scratch buffers/`Stiffness`), re-enters `this->setTrialStrainIncr()` for each of the 6 (first-order) / 12 (second-order) perturbations, restores after every one, propagates any perturbed refusal outward immediately, and assembles the FD into a LOCAL matrix (several `Backward_Euler` early-return branches assign `Stiffness` unconditionally and would clobber already-written columns otherwise). `compute_local_stress()` has zero remaining callers (grep-confirmed) and is kept, re-tagged dead/not-a-map per D2. Four non-smooth switch sites that already hard-route `Numerical_Algorithmic_*` to the analytical `Stiffness` before reaching `ComputeTangentStiffness()` are left alone (FD across a non-smooth apex/corner blends two active sets). **Measured** (build c24cda99c, re-verified on 7e93e4381): `Backward_Euler` + `Numerical_Algorithmic_SecondOrder`/`FirstOrder` vs `fd_check` of the binary's own residual **2.055e-08 / 3.394e-08** (both far past the task brief's own `<=1e-6`/`~1e-3` expectations — the single-step uniaxial rig's converged strain increment keeps both stencils' truncation error tiny); `Closest_Point` `Algorithmic` vs `Numerical_Algorithmic_SecondOrder` **1.209e-08** (the ADR-97 D2 cross-check, now genuinely a cross-check rather than a competitor); the ADR-94 two-cube `testIter` sum for `Backward_Euler`+`Numerical_Algorithmic_SecondOrder` **16** (matches `Closest_Point`+`Algorithmic`'s recorded 16 exactly, down from `Backward_Euler`+`Continuum`'s recorded **113**); H6's numerical pair moved from **4.6%** (wp/94c) to **~2-3e-8** (six orders of magnitude); the `Continuum`-vs-`Numerical_Algorithmic_FirstOrder` component pin for VonMises moved from **0.0197** to **0.2196** (an order of magnitude LARGER, because the option now tracks `Backward_Euler`'s real multi-iteration cutting-plane answer instead of the old third map's coincidentally-smaller disagreement — re-pinned as a hard lower-bound floor at 0.1). `Backward_Euler`'s own source is untouched (grep-confirmed no call to `compute_local_stress` inside its body — D1 automatic). Gate 4 re-verified with ONE deliberate baseline regeneration: `cube/vm/BE/Numerical_Algorithmic_FirstOrder/plastic` moved by up to 5.428e-09 absolute — root-caused (not assumed) to the deck's LOAD-CONTROLLED/free-DOF rig, where `Backward_Euler` calls `ComputeTangentStiffness()` at the tail of every outer-Newton trial, so a materially different returned tangent shifts the outer solver's convergence PATH and its converged point within `NormDispIncr` tolerance, at the same order of magnitude as the file's own documented cross-platform compiler-noise floor (5.8e-09); the paired `.../elastic` leg of the SAME deck (exactly linear, no path-dependence) and all 22 other decks re-verified bit-identical. **Refusal propagation:** the scope's ideal reproducer (a perturbation ALONE starving while the primary converges) was found impractical to engineer — measured, a 1e-8-relative FD perturbation essentially never changes a well-converged Newton's own iteration count, even bisected to the EXACT boundary a deck needs with zero slack. Replaced with two reliable reproducers: `Backward_Euler` + plain MohrCoulomb on the ADR-84 P2a exhaustion deck (`n_max_iterations 2`, `strict_convergence 1`) refuses identically under `Secant` and `Numerical_Algorithmic_SecondOrder`; `Closest_Point` + VM under the ADR-97 P1 gate-6 starved reproducer still refuses under `Numerical_Algorithmic_SecondOrder`. Full explicit-list battery (`test_adr84*`, `test_adr94*` minus `test_adr94_matrix.py` execution, `test_asdplastic_*`, `test_adr97*`): **236 passed, 2 skipped, 0 failed**. **gate 5 (mutation, [[reviews/adr97_p4_mutation]]):** reverting the two thin wrappers to their pre-P4 `compute_local_stress()`-based bodies on a scratch build reproduces the historical defect almost to the digit — FD error **4.574e-02** vs the pre-P4-recorded 4.6% — killing 4 of 6 `test_adr97_p4_numalg.py` tests plus H6, while the two refusal-propagation tests (correctly testing an orthogonal property) stay green; reverted, rebuilt, re-measured identical. All findings in [[LEDGER_quirks]] (the outer-Newton-tangent-path lesson and the FD-perturbation-can't-starve-Newton lesson). Full report: [[reviews/adr97_p4_report]].
- **2026-09-08 — P6 (`wp/97g-measure`, PR [#826](https://github.com/nmorabowen/OpenSees/pull/826), based on `wp/97c-cp-principal` = P2 [#824](https://github.com/nmorabowen/OpenSees/pull/824)).** The D1 mesh-scale measurement: a strip-footing bearing-capacity model, `LadrunoBrick` hexes, gravity-initialised then pushed to failure, five configurations ({`Backward_Euler`,`Closest_Point`} x {`Secant`,`Continuum`,`Algorithmic`} x algorithm, `system UmfPack`) at two mesh scales (3993 and 24000 DOF, the ADR-80 hex8-class rung) on two non-associated material families (a generic MC deck and the Cerro-Lindo-like MCTC EDZ deck from `test_asdplastic_mctc.py`). Every configuration runs in its own fresh `python3.12` child process (the per-tag option-map lesson, reused from P1/P2). **Measured:** `Closest_Point` converges strictly MORE of the load history than `Backward_Euler`/`Secant` under identical `NormDispIncr` criteria on every family/scale combination — MC **10/10 vs 7/10** (3993 DOF) and **12/12 vs 7/12** (24000 DOF); MCTC **9/10 vs 6/10** and **11/12 vs 6/12** — at equal-or-fewer total Newton iterations even over the steps `Backward_Euler` DOES complete (e.g. MC 24000 DOF: `CP_Algorithmic` 48 iterations over 12 steps vs `BE_Secant`'s 56 over only 7 before stalling); `CP_Algorithmic_Krylov` (the fork's default algorithm) is fastest by wall clock at every scale/family measured. The committed-stress gap between the two maps on steps both complete stays under 1% (0.02%-0.68%, shrinking with mesh refinement — the expected trend for the known small divergence on paths with mild flow-direction rotation, not a correctness gap). `BE_Continuum`'s iteration count is consistently lower than `BE_Secant`'s at the SAME converged-step count (ADR-94 M3 reproduced at mesh scale), but neither `Backward_Euler` tangent choice recovers the steps it cannot converge — the failure is the cutting-plane MAP, not the tangent quality feeding Newton. **Recommendation:** flip the default to `Closest_Point` + `Algorithmic`, `algorithm` unchanged, scoped to the 23/46 supported specializations — the owner's decision, a separate PR, full caveats in [[reviews/adr97_p6_measurement]] §6 and restated in the verdict §3. **Test coverage:** `tests/test_adr97_p6_measure_smoke.py`, a 144-DOF smoke mesh (Zone-A, <1s) asserting every configuration converges and `CP_Algorithmic`'s total iteration count does not exceed `BE_Secant`'s — the ordering this WP measures at mesh scale, robust to build-to-build float noise. Full report: [[reviews/adr97_p6_measurement]].
- **2026-09-08 — P7 (`wp/97h-closeout`, this PR, based on `wp/97e-numalg-repoint` = P4 [#829](https://github.com/nmorabowen/OpenSees/pull/829) with `wp/97g-measure` = P6 [#826](https://github.com/nmorabowen/OpenSees/pull/826) merged in, build `7e93e4381`, SRC-identical to HEAD `5e89b8a12`).** The ADR-87 warrant package: [[reviews/adr97_verdict]] (§0 verdict — warrant-grade for 23/46 specializations, opt-in, one documented byte-identity exception root-caused to an outer-Newton-path effect on a load-controlled rig, not a code-path regression; §1 a per-gate/per-WP verification manifest with Zone-A run ids, including the gate-4 platform-scoping fix `94abcdbb7` — the first Linux runs of P1/P2/P6 failed on a `1e-9`-style pin before it merged up; §2 the four mutation-gate records consolidated; §3 the P6 table restated with the D1 recommendation stated explicitly as the owner's decision; §4 the six owner-facing findings not fixed under D1, each pinned by name; §5 residual risks; §6 the merge-order guide (`#817→#819→#824→#825→#827→#829→#826→`this PR, retarget-to-`ladruno`-after-each-merge, re-dispatch Zone-A after every retarget, merge commits only); §7 the six orchestration lessons this ADR's multi-session execution surfaced). Banner line added (`Ladruno_scripts/banner_features.txt` + `patch_banner.py` regen) and verified against a fresh build. Ledgers consolidated: `LEDGER_implementations.md`'s three existing P1-P3 rows plus new P4/P5/P6-closeout rows; `LEDGER_vanilla_files.md` verified complete against `git diff --stat origin/ladruno...HEAD -- SRC` (23 files, all with a `// Ladruno (ADR-97 ...)` marker and a ledger row); `LEDGER_quirks.md` gained the gate-4 platform-scoping entry and the six orchestration-lesson entries. Full explicit-list battery on the closeout build: **237 passed, 2 skipped, 0 failed** (`test_adr94_matrix.py` execution deliberately excluded, per its own quirks entry; `git status` clean before and after), re-confirmed on the post-banner build (`33c670bf5`, `ops.ladrunoBuild()` verified, banner splash confirmed printing the new line). One Zone-A run (#829, P4, 34188231854) was still in progress when the verdict's manifest was first drafted; re-checked after the build/battery and resolved `success` — all seven branches in the stack are green.
...
[[reviews/adr97_verdict]] (the P7 warrant package)
