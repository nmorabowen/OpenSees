...
- **2026-07-05 — Live console monitor, #485 (logged 2026-09-25 by WP-125).** `Ladruno_tools/profiler_viewer/profiler_monitor.py`: `ProfilerMonitor` wraps an `analyze()` loop (or `start`/`step`/`stop` around the caller's own loop) and shows, live, per-step iterations + `testNorms` residual + dt, wall per step / steps/s / ETA, profiler peak-memory growth, and every N steps the cumulative phase shares from a throw-away mid-run `profiler('report')` read through `ProfilerResults`. It uses only the shipped `profiler` command and standard openseespy introspection: no C++ change, no rebuild. `step()` returns `False` on a nonzero `analyze` code. `monitor_smoke.py` is the binary-free regression (a fake `ops` whose `report` writes a real untimed-`root` rollup, so #484's backfill is exercised too). This is not the Analysis monitor's viewer (`recorder Monitor`, [[08_analysis_monitor]]); that one is `Ladruno_tools/monitor_viewer`.
- **2026-07-04 — Real-engine rollup: untimed `root` anchor backfilled, #484 (logged 2026-09-25 by WP-125).** The C++ writer roots the phase tree at an untimed `root` (`wall_ns=0`, `calls=0`) whose child is `step`, while `make_sample.py`, which every Python/React test used, has a timed top node. So on every real run `share` was normalized by `wall_ns or 1` = 1 ns, and the icicle (`wall_ms <= 0` → return) drew no frames. Fix in `Ladruno_tools/profiler_viewer/profiler_results.py`: `rollup()` backfills the anchor's wall as the sum of its children and uses it as the share denominator (root `share` = 1.0). Backend only, no C++ change. The regression `_untimed_root_rollup()` in `test_contract.py` fails on the old `rollup()` and passes with the fix.
