...
- **P2 — Phase seams.** Scopes in `DirectIntegrationAnalysis`/`StaticAnalysis` (`analyze`/`analyzeStep`/`commit`), `NewtonRaphson::solveCurrentStep` (6 sites), `Linear::solveCurrentStep` (4 sites). Yields the full phase tree for implicit + explicit. **+P0#3** wire per-step `iters`; **+P0#5** dt/dt_cr/oversample into the run header.
- **P3 — Assembly aggregates (deep).** `OPS_PROFILE_SCOPE_DEEP` around the element loops in `IncrementalIntegrator`/`TransientIntegrator` `formTangent`/`formElementResidual`. **+P0#1** per-element-class buckets keyed by `getClassTag()` (+ `fb_coupled` tag); **+P0#4** ExplicitBatheLNVD sub-step scope placement (`:424-425`).
- **P4 — Memory counters.** Per-thread (not global-atomic, P1#9) counters in `Matrix`/`Vector`/`ID` ctor+dtor; `TaggedObject` census; static-scratch whitelist.
  - **DONE (Matrix + Vector), PR #46.** `AllocType`-tagged `countAlloc/countFree` (`OPS_PROFILE_COUNT_ALLOC/FREE(bytes, type)`) wired at every per-object `new[]`/`delete[]` in `Matrix.cpp`/`Vector.cpp` — alloc unconditional after `new[]`, free under the `fromFree==0` ownership guard before `delete[]`, so external-data views and moves stay balanced; the static `matrixWork`/`intWork` scratch is whitelisted (not counted). `buildMemorySnapshot` lowers the per-thread `liveBytesByType[]` into the snapshot's `matrix_live`/`vector_live` + aggregate `peak_bytes`. Smoke `Ladruno_tools/profiler_viewer/mem_smoke_{model,check}.py` (20-truss static): `matrix_live=160, vector_live=384, peak_bytes=544`, balanced.
  - **Deferred:** **ID** counters — `ID::operator=` overwrites `arraySize` before its matching `delete[]` (needs a saved old-size temp), and ID has several realloc paths (`unique`/`insert`/`operator[]`/`resize`/`operator=`); `id_live` stays 0 until the follow-up PR. **TaggedObject census** (`components_live`) — separate mechanism + component-lifecycle/whitelist surface; the writer already emits an empty census dataset. **Known approximation:** `Vector::resize` shrink-keeps its buffer while lowering `sz`, so a later free under-counts the shrunk delta (rare; `liveCount` stays exact).
...
- **P7 — Python backend.** `ProfilerResults` loader (`h5py`) + normalizers + `diff(base, cand)` (walk rollup groups by path) + FastAPI endpoints (`/runs`, `/runs/{id}/rollup`, `/series`, `/diff`). This is also the headless analysis API (usable from Jupyter without the frontend).
- **P8 — React frontend.** Flame/icicle (rollup), time-series (series), run-comparison/diff table + overlay, dt_cr/oversample/leak badges. Consumes the P7 JSON API; never reads HDF5 directly.
