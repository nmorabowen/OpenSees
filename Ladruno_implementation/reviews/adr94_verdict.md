...
| **M4** | **H10a** `HoekBrown_YF` is one commit behind `jaabell/ASDP` (`60d9b9b23`): our if/else is discontinuous at `arg = 0`; a uniaxial tension path locks onto `σci·s` = 587 kPa instead of `σt` = 245 kPa (factor `mb` = 2.4) then stalls. Compression paths bit-identical between trees. Port direction: **take jaabell's composite** (D4). | major (rock in tension) | RESULTS | `test_adr94_hlist_hb::test_H10_hb_*` (3) |
