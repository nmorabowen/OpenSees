...
| **B4** ✅ | **[FIXED `wp/94a-fail-loud` (NaN) + `wp/94c-numerics` (gradients, apex)]** **H10b + red-numerics Q5** Drucker–Prager hydrostatic tension through the apex commits **NaN with `analyze() == 0`**. Root cause is two-part: the NaN originates in `VoigtVector pressure_part; pressure_part *= 0.0;` on uninitialised Eigen storage (`DruckerPrager_YF.h:64-65`, `DruckerPrager_PF.h:68-69`, the ADR-84 constructor trap again; also `NullHardeningTensorPolicy` and AF's saturation branch), and the BE NaN guard *does* fire with a bare −1 that the host drops (B2). The apex-return body in BE (2093–2161) is commented out for every YF; `RoundedMohrCoulomb_YF` declares the apex trait without defining either method. **wp/94c:** the analytical gradient error is fixed — `d sqrt(J2)/d v` is `r/(2 sqrt(J2))` on the normal slots, not `r/sqrt(J2)` — taking the component FD error from **0.971 to 1.15e-08** and the `tet/dp/BE/default` committed `\|f\|` from 9.2e-07 to **7.3e-12** (stress moved 2.7 %); `DruckerPrager_YF::CHECK_APEX_REGION`/`APEX_STRESS` are implemented (they were `Implement!!!` stubs); the BE apex call site is LIVE with an `f(sigma_apex) <= tol` admissibility gate, so hydrostatic tension now returns to `p = xi_c/eta` and completes 40/40 instead of being refused; `MohrCoulomb_YF`'s region test is the normal-cone form; `RoundedMohrCoulomb_YF`'s undefined trait is dropped. | blocker (DP in tension) | RESULTS, NaN | `test_adr94_hlist_hb::test_H10_dp_apex_*`, `test_adr94_redblue_numerics::test_R2_dp_apex_nan_guard_fires_and_is_swallowed` |
| **B5** ✅ | **[FIXED `wp/94c-numerics`]** **H16 (new, R3a)** Shear-slot derivative convention is split. `VonMises_YF`/`_PF` return the tensor derivative (shear slot = ∂f/∂σ12); MC, HB, MCTC, StiffSoil return the Voigt derivative (2×); `DruckerPrager_YF` matches neither (0.97 relative error on normal slots, a genuine gradient error). Consumption is split the same way: BE contracts with `tensor_dot_stress_like` (doubled shear), the other five sites with a plain Voigt dot, and every site accumulates `dLambda*m` and `Eelastic*m` expecting engineering-shear `m`. Net: VM flow direction under-counts shear at 5 of 6 sites and in the plastic-strain update; MC-family over-counts shear in BE's consistency scalar (convergence rate only, since BE iterates to Φ = 0). **wp/94c:** ONE convention, VOIGT everywhere. VM YF/PF double their shear slots; DP's normal slots are halved; BE's plastic modulus is a plain contraction, matching the other five sites; the hardening equivalent norms use the matching helper (new `tensor_dot_engineering_strain_like`). **The unmeasured magnitude is now measured:** a VM simple-shear plastic step agrees with the closed-form radial return to **1.47e-13** relative (`adr94_oracle/vm_shear_oracle.py`), against O(1) before; VM `Backward_Euler` halved its iteration count (21 → 11); the `Numerical_Algorithmic` tangents went 31 % → 4.6 % from the consistent tangent. | major → blocker-candidate | RESULTS on shear-dominated VM/DP paths (**measured**: 2.7 % stress on `tet/dp`, 1.5e-13 residual on VM shear after the fix); COST for MC-family in BE | `_adr94_components.md` "Convention adjudication" (analytic, exact); `test_adr94_components` (VM Continuum-vs-numerical 1.4–1.8 %) |
| **M1** ✅ | **[FIXED `wp/94b-statics`]** **H1 + F1** Class-static `Stiffness` (and `dsigma`, `depsilon_elpl`, `intersection_*`): every GP, element and tag of one specialization is assembled with the tangent of the last GP integrated; `getInitialTangent()` mutates it as a side effect. Two disconnected elements with tangents 13.6 % apart assemble bit-identical blocks. Converged results are exact (blue: 1e-6 agreement with each cube solved alone); cost is 62 % more iterations on that model and non-convergence on harder ones. Permanent ADR-75b threading blocker; the YF/PF `static vv_out` buffers (F2) widen the sharing to every combo that reuses a functor type. | major (COST), blocker for threading | COST; RESULTS only via cutback history | `test_adr94_hlist_numerics::test_H1_*` (3), `test_adr94_redblue_blue` (2), `test_adr94_redblue_cpp` |
...
| **M5** ◉ | **[FIXED (OPT-IN) `wp/94c-numerics`]** **Tolerance is absolute in stress units** (red Q2). Default `f_absolute_tol 1e-6` against `\|Φ\|` scaled by σy / c·cosφ / σci·sᵃ: the same MC problem passes 20/20 in kPa and is refused on step 1 in Pa. HB's central-difference normal through the discontinuity adds ~3e6 of spurious gradient inside `HB_ds`. **wp/94c:** `f_relative_tol` (default **0 = off, byte-identical**) makes the tolerance `max(f_absolute_tol, f_relative_tol * yf.strength_scale())`, each YF declaring its own scale; with it on, the same problem completes 20/20 in both unit systems. The SHIPPED default is still absolute, so the finding stands for anyone who does not opt in — hence ◉ not ✅. Note the reproducer moved out by six orders (x1e3 → x1e9) because B5's contraction fix also made BE converge much harder on that deck. | major | RESULTS (refusal vs acceptance decided by units) | `test_adr94_redblue_numerics::test_R2_f_absolute_tol_makes_strict_convergence_unit_dependent` |
| **M6** ✅ | **[FIXED `wp/94b-statics`]** **H4 + F4** `revertToLastCommit()` is a no-op; `revertToStart()` returns −1 that `Domain::revertToStart()` and `OPS_resetModel` ignore. `ops.reset()` leaves the material inconsistent with geometry (a third stress value, neither zero nor the pre-reset commit). Cutback-recovery divergence measured at ~6e-9, inside Newton tolerance (inconclusive). | major | RESULTS after `reset`; latent on fixed-step decks | `test_adr94_hlist_mechanical::test_H4_*`, `test_adr94_contract` (revert tests) |
...
| **m3** ◐ | **H2** `getClassType()` returns a dangling `c_str()` (SSO luck) — NOT fixed. **H14/F6 [FIXED `wp/94b-statics`]** `getCopy()` omits `first_step` (latent: hosts copy at construction). The parser builds 46 throwaway instances per `nDMaterial` call. | minor / doc | none observed | reading; `test_H14_*` (structural) |
...
| `wp/94e-revert` **[SHIPPED in `wp/94b-statics`]** | Implement `revertToLastCommit`/`revertToStart` (Trial ← Commit, IVs revert, `first_step` reset). Folded into wave 2 because it touches the same file and the same constructors as the per-instance conversion. `revertToStart()` restores the IVs from a snapshot taken on the instance's first `setTrialStrain` (`utuple_storage` has no initial-value facility). | `test_H4_*`, contract revert tests |
...
| Item | Content |
|---|---|
| H1/F1/F2 **[SHIPPED in `wp/94b-statics`]** | Per-instance `Stiffness`, `dsigma`, `depsilon_elpl`, `intersection_*`; `getInitialTangent()` without side effect; YF/PF `vv_out` per-instance. Done as one pass over 19 files (30 statics), with the bit-identity gate run as 23 single-element decks / 280 committed-stress rows dumped from the pre-change build and diffed against the post-change build. Also caught: `ElasticityBase::EE_MATRIX` (the same defect one layer down) and the seven per-integrator `static VoigtVector depsilon` locals. **ADR-75b threaded assembly is no longer blocked by ASDP's statics**; the vanilla `static Vector result(6)` / `static Matrix return_matrix(6,6)` return-by-reference buffers remain, as in every `NDMaterial`. Owner authorised the vanilla edit; the jaabell coordination is a port-back, not a precondition. |
| B5 **[SHIPPED in `wp/94c-numerics`]** | Pick one shear-slot convention for `df_dsigma_ij`/`pf` and one contraction at every site; fix DP's normal-slot gradient. Done: VOIGT everywhere, plain contraction at all six sites, `tensor_dot_engineering_strain_like` for the hardening norms. The numpy radial-return oracle exists (`adr94_oracle/vm_shear_oracle.py`) and the missing runtime magnitude is **1.47e-13** relative agreement on a simple-shear plastic step (O(1) before). Owner authorised the vanilla edit; the jaabell coordination is a port-back, not a precondition. |
| M5 **[SHIPPED (opt-in) in `wp/94c-numerics`]** | Relative tolerance (`\|Φ\|` scaled by the YF's own strength scale). `f_relative_tol`, default 0 (off, byte-identical); `YF_STRENGTH_SCALE` on VM/DP/MC/MCTC/HB/StiffSoil/TensionCutoff with an inert base default. The default remains ABSOLUTE — changing it is a fork default change and was not taken here. |
| B4 apex **[SHIPPED in `wp/94c-numerics`]** | Revive the apex call site + implement DP/RoundedMC apex methods. Done: the site is live with an `f(sigma_apex) <= tol` gate (fall through to the generic return map, or refuse under `strict_convergence`); DP's two methods implemented; MC's region test upgraded to the normal cone; HB's inverted sign fixed. **RoundedMohrCoulomb's trait was DROPPED, not implemented** — it is not registered (`AllYieldFunctions.h` keeps its `#include` commented out), so there is nothing to test an implementation against; re-add the trait in the same change that adds the methods. Residual, recorded in `LEDGER_quirks`: `check_apex_region` is a Euclidean normal-cone test while the exact condition is in the elastic metric, and the `f(sigma_apex)` gate cannot catch a misclassification. |
