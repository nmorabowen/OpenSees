...
| `SRC/classTags.h` | Register `RECORDER_TAGS_LadrunoMonitorRecorder`=33002 (live analysis-monitor recorder) | TBD (analysis-monitor) |
| `SRC/interpreter/OpenSeesOutputCommands.cpp` | Register `recorder Monitor` keyword → `OPS_LadrunoMonitorRecorder` (HDF5 block) | TBD (analysis-monitor) |
| `SRC/recorder/CMakeLists.txt` | Add `LadrunoMonitorRecorder` + `LadrunoMonitorSink` to the recorder target (HDF5≥1.12 block, SWMR) | TBD (analysis-monitor) |
