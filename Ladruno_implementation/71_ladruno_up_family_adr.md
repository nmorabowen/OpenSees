...
- **Static well-posedness has two necessary conditions**: k̄ > 0 and **≥1 drained
  ...
  first thing a user will build). Parser/guide warn. **The failure is SILENT, not loud**
  (measured 2026-07-11; `tests/test_ladruno_up_element_analytic.py`, strict xfail
  `test_all_impervious_static_fails_loudly`): every serial general solver — UmfPack,
  FullGeneral, BandGeneral, SparseGeneral — factorises the structurally singular
  system through round-off and returns `rc = 0` with an arbitrary, solver-dependent
  pressure level; the p-RHS is consistent, so no solver reports the rank deficiency.
  The singularity itself is pinned by the companion test
  `test_all_impervious_static_tangent_is_singular`; the guard is the user's datum
  (apeGmsh build gate G4 checks it), not the solver. Conversely,
