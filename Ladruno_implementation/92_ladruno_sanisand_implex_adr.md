...
- **2026-09-14 — F10, the SELF-WEIGHT wall: `-implexControl` is what stops the deck.**
  ...
  strip — is reproduced (`FLOOR` at 0.0085, 724 refusals, **all** in the `control` bucket,
  `implexGuards[0] = 0`, companion 0). **The decisive arm is the one with the control OFF:**
  ...
  **aggravator of the error field** (100 % of points confirmed) but not the wall — the same field
  ...
  becomes "the CONTROL's error control, not the substepper's"; the `nu*` device is **refuted**
  (at `K0 = 0.455` `nu*` IS the material's own `nu`; the leg-`C` half of that argument is
  withdrawn as not one-variable). **`tol = 0.5` agrees with the control-off arm to 0.395 % mean /
  1.625 % max over `0.002 ≤ s/B ≤ 0.05`, i.e. it makes the control nearly inert on this deck.**
  Two defects recorded, not fixed — `implexPrimed`'s sign test (now the named mechanism of the
  FLOOR, fix shape = P2-5b's relative test), and the P2-2 `f = 0` guard fighting the control
  (30 of 49 throttled refusal lines are at `f = 0`); §10 of the note states the design question
  for this ADR to settle. Reporting hygiene added: a refuse/halve/regrow path **stiffens** the
  curve (+19.4 % at the settlement where leg B dies), so no `q` from a refusing leg is
  comparable.
...
| P2-9 | **The P2-2 guard is a threshold at zero, and it is reactive** | measured on Esmeralda (#807 acceptance): the guard is (a) SANISAND-specific in its trigger (`Kp <= 0` / an `alpha_in` reset), (b) a threshold that UNDER-fires — the engine that fired at ~2x the points by accident reached 10 % further on the twin's curve at no accuracy cost (honest wall 0.0177 vs 0.0169), and (c) reactive: it reads the committed predecessor, so a trial that FIRST reaches a softening point is extrapolated at full `f`, and P2-6's retry rescues only where the elastic predictor already passes tol (322 rescues against thousands of refusals) | replace the guard's *degree* and *timing* with a closed-form, material-agnostic factor chosen from the companion the control already computes: `sigma~(f) - sigma_impl = A - f*B` with `A = sigma_n + Ce:d_eps - sigma_impl` and `B = Ce:d_eps_p(n)`, so `f* = clamp((A:B)/(B:B), 0, f_max)`, `f_max` = today's clock ratio. Computed at the FIRST trial of the step and FROZEN for it (the global step stays linear); `B:B = 0` leaves `f_max` standing. Flag `-implexFactor fixed\|control\|controlIter`, default `fixed`; `control`/`controlIter` REQUIRE `-implexControl` (no companion at the trial without it); `controlIter` recomputes `f*` at every Newton trial from that trial's own `d_eps` instead of freezing it (priced alternative below the table). **Precedence: the P2-2 guard is not bypassed — it runs first and unconditionally, so when it fires `f_max = 0` and the clamp can only return 0; `f*` replaces the guard's degree only where the guard had nothing to say, one step earlier.** Reported in `implexDetail[5]` (the `f` actually used) and counted in a new `implexGuards[6]` (`f* < 0.5*f_max`, "backed off") | pre-registered in `_adr92_p2_9_control_informed_f_plan.md` §2: G0 oracle rows byte-identical where `B:B = 0`; seat replay (step 331) error 0.46 -> <= 0.05; fork R3 registered arm depth >= 0.076 with overlay <= 2 %; **Esmeralda dense refuse arm honest wall >= 0.0177 on the twin within 0.3 %** (the decision rule); loose arm unchanged at 0.039 (material, ADR 93) | **CLOSED 2026-09-08; measured graded guard, not default: dense wall 0.01755 vs bar 0.0177** — C++ + tests in `wp/92f-implex-control-f` (`a6a53948e`/`9e73060a9`/`aeb1da492`, merged `179da6ffb` PR #822); `-implexFactor fixed` is the default and is byte-identical to the pre-P2-9 build. Oracle seat prediction PASSED (0.4625 -> 0.00082) but surfaced a first-iterate elastic-predictor bias the plan did not name. Fork R3 registered arm then REFUTED `control` (depth 0.052 < 0.076, overlay 11.1 % > 5 %) on exactly that bias, and PASSED `controlIter` (depth 0.115, overlay 1.70 %) at ~13x the wall time and two orders of magnitude more Newton max-iteration stalls. TIMs' Esmeralda dense-refuse arm (`controlIter`, leg 146607) then measured the honest wall at **0.01755** — above the refutation bar (0.0169) but 0.85 % short of the ship bar (0.0177), so per the pre-registered rule's otherwise branch **P2-9 does not ship as default** (no code change owed; `fixed` was already the default) and `controlIter` is recorded as a graded guard with its measured gain (reach +3.9 %, refusals 42 545 -> 102, failed attempts 248 -> 11) against its measured cost (~2.9x wall time on this deck, NOT the R3 leg's ~13x). `control` stays REFUTED; P2-8's fixed threshold remains the documented fallback. See "P2-9 findings" below the table and `_adr92_p2_9_esmeralda_results.md` |

**Measured trade of the corrected guard (Esmeralda 887fea475, dense refuse arm 146591 vs 87b9cf846's 146570):**
the curve stays on the implicit twin (within 2 % all the way; 1812 kN at s/B 0.0161 is the twin's value) but the
honest wall arrives at **0.0161 instead of 0.0177**, with the `f = 0` guard firing 29 527 times against 64 642: on
the earlier engine the guard's misfire on the first commit (P2-2b's target) put the elastic predictor at more points
...
**P2-9 findings (oracle + fork R3, 2026-09-07/08):** the numpy oracle
(`_adr92_p2_9_oracle_results.md`) passed the pre-registered seat prediction with room to
spare (step 331 error 0.4625 -> 0.00082, the plan asked for <= 0.05) and confirmed
byte-identity on the `B:B = 0` rows (G0/G2), but found a failure mode the plan did not
name: `f*` frozen on the drained-triaxial sweep's first iterate is 100-450x *worse* than
today's `f` (the first iterate is near-elastic, so `A` carries almost no signal and `f*`
collapses toward 0), while `f*` recomputed on the converged `d_eps` is 1.0-2.1x *better*.
The fork's Fork R3 registered arm (`_adr92_p2_9_r3_results.md`) reproduced the same
mechanism on a real BVP: `-implexFactor control` (frozen f*, build `a6a53948e`) is
**REFUTED** — depth 0.052 against the 0.076 bar, overlay 11.1 % mean deviation against
the 5 % refute threshold, `implexGuards[6]` backing off 196k times over 366 steps.
`-implexFactor controlIter` (f* recomputed every trial, build `9e73060a9`) **PASSES**
both bars — depth 0.115, overlay 1.70 %, actually the best overlay of any arm measured
in this campaign including plain `fixed` — but at ~13x the wall time (1858 s vs 141.6 s)
and a jump from 1 to 89 Newton max-iteration stall markers, plus more (not fewer)
material refusals per converged step (2.31 -> 7.39). Net: P2-9's closed-form factor is
real and its mechanism is confirmed by two independent measurements, but only the
per-iterate form clears the acceptance bars, and its cost means the Esmeralda
dense-refuse arm decided whether it ships. `fixed` remains the default and the thing to
reach for.

**Esmeralda dense/loose-refuse arm, closing measurement (2026-09-08, engine `179da6ffb`,
`_adr92_p2_9_esmeralda_results.md`):** dense honest wall on `controlIter` (leg 146607,
control 0.1/0.01) = **0.01755** against the P2-7c fixed-f reference wall 0.01689 — a
**3.9 % reach gain** — with overlay comparable-to-better (+0.28 % mean vs +0.31 %) and a
collapse in refusal churn: refusals 42 545 -> 102 (~417x fewer), failed attempts 248 -> 11
(~23x fewer). Cost: **~2.9x wall time on this deck** (1 933 s -> 5 620 s at 17.2 it/step)
— explicitly NOT the R3 registered arm's ~13x, which does not generalise past its own
smaller deck. Against the pre-registered decision rule (ships at >= 0.0177, refutes at <
0.0169), 0.01755 clears refutation but misses ship by 0.85 %, landing in neither branch of
the `if` — so the rule's **otherwise branch is decisive: P2-9 does not ship as a default**
(`fixed` was already the default, so no code change is owed by this wave); `controlIter`
is recorded as a graded guard with the gain/cost above, staying an explicit opt-in for the
TIMs campaign; P2-8's fixed threshold remains the documented fallback. A companion
tol-0.01 dense leg (146608) reaches further (0.01932) by spending its full subdivision
budget rather than refusing honestly, but sits +1.44 % HIGH against the twin (stiffer, not
closer) — a finding, not a candidate configuration. The loose arm (146609) ends at
0.03921, unchanged at 0.039 to the third figure exactly as pre-registered — PASS, not a
finding.
