...
|P2-7|**the stage flip absorbs the drift correction immediately.** Measured on the R3 footing (1600 GPs, ds 5e-5, default flags): the first plastic step after `updateMaterialStage 1` commits with `implexError` max **0.239** (exempt from refusal as the un-primed step) against 0.02–0.03 on every later step, and **0.051** when a hold precedes it — the newly plastic committed state is un-corrected, the un-primed elastic predictor is measured against a companion that first snaps it, and the step commits an O(0.2) equilibrium gap. On Esmeralda's dense deck step 2 then refuses and the P2-6 legs on d5bd259f6 ran 17–20 % soft from step 2 (walls at 0.013); on 87b9cf846 the P2-2b bug's accidental `f = 0` on step 2 closed that gap and overlaid the implicit twin to 0.2 %. The explicit `-implexGuard/-implexTrialGuard/-implexFloor` words are innocent (each consumes one token; defaults identical)|under `-implex`, the flip runs a zero-increment companion return from the committed state and commits it as a hold-preserved commit (history zero, no guard flags, counted in `implexGuards[5]`)|building; acceptance: fork probe step-1 error ≈ 0.05; Esmeralda dense P2-6 legs give ~9.9 kN at step 2 with no refusal and overlay the twin; `implexGuards[5]` +34 560 at the flip||||
