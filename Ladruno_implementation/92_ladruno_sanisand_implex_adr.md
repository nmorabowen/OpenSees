...
|P2-7|**the stage flip absorbs the drift correction immediately.** the IMPLICIT path is 23–33 % soft from step 1 on every engine since P2-5 (Esmeralda 146574/146586: first rows 1.296 / 2.6 kN vs 6.511 on c162833ed; step 1 subdivides). Source: the elastic stage sets `α = dev(σ)/p` every step (`ManzariDafalias.cpp:1055`) so at the flip `α = r_gravity ≠ 0`, `α_in = 0`; `Elastic2Plastic()` never touches `α_in` (init lines `:5139-5141` commented out); the only initialisation is the sign test in `integrate()` `:1005-1013` on the first plastic evaluation, which on a zero-increment re-equilibration is round-off noise — vanilla initialised `α_in := α` at roughly half the points by chance; P2-5/5b/5c turned that into never, so `(α − α_in):n` starts large and `Kp` small. The first P2-7 attempt (hold-style skip at the flip, 691f4064d) would have done the same and was not built.|at the flip `α_in := α` deterministically at every point on both paths (`-flipAlphaIn init|vanilla`, default `init`; `vanilla` reproduces the old noise for A/B), the reversal-noise guard applies only to PRIMED states (after the first plastic commit since the flip), and under `-implex` a zero-increment companion return at the flip absorbs the drift (history zero, counted in `implexGuards[5]`)|built in #807 (887fea475); fork probe + battery running; acceptance: fork probe step-1 error ≈ 0.05; implicit twin's first-step stiffness ≥ c162833ed's (a MODELLING change on the vanilla twin — report it as such; the `vanilla` flag reproduces the old number); Esmeralda dense P2-6 legs give ~9.9 kN at step 2 with no refusal and overlay the twin; hold census passes at every hold|||| **Default pending the owner's decision (TIMs RC14):** the fork ships `init` because the noise initialisation is a defect (two identical decks can differ 20–30 % in early stiffness by round-off sign), but the vanilla twin's charter in the TIMs act is "vanilla's number", so `init` makes `LadrunoSANISAND` a different model from real `ManzariDafalias` at the flip; `-flipAlphaIn vanilla` reproduces the old number exactly, and every P2-7 curve names the flag in its header. |
