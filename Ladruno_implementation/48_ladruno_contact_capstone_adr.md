...
| Perpendicular edge-edge (cos_t→0) narrow phase | `SRC/domain/contact/LadrunoEdgeKernel.h` (**E0**) + `LadrunoContactFE` `EDGE_EDGE` mode (**E2**) + `LadrunoContactDomain::EdgeEdgeState` (**E2**) + handler routing + `-edgeedge` (**E2**) | 🔄 in progress — **E0 (geometry) SHIPPED** (`LadrunoEdgeKernel.h`: Ericson clamp ladder + common-perp normal w/ body-fixed committed sign + gap + B-operator; oracle 28/28 + `e0_cpp_check.cpp` 13/13 bit-for-bit). **E1 (routing logic) ORACLE SHIPPED** (`proto_e1_router.py` 16/16; the align_fn regime-split precedence — caught oracle-first that flat NTS-precedence steals facing-facet contact). **E2 (penalty NORMAL force + main tangent) SHIPPED — first live C++/build phase**: a 4th `LadrunoContactFE` mode `EDGE_EDGE=4` (stateless 4-node adapter, **no new class tag**; `f=t_N·B`, `t_N=ε_N⟨−g_N⟩`, main tangent `K=ε_N·BᵀB`), Domain-owned `EdgeEdgeState` (composite-key, body-fixed committed sign, **commit()/revert() extended to iterate it** — the explicit obligation), handler routing loop over the **brute-force** facet pairing (the E1 precedence, now non-inert) injecting one adapter per kept boundary-edge pair, `-edgeedge` opt-in on `-mortar` (OFF ⇒ byte-identical; perpendicular pair's mortar adapter geometrically inert ⇒ no double-count). **E3 (friction) SHIPPED** — the 4th consumer of the shipped `LadrunoFrictionKernel` (NO new friction code): `edgeSlip` builds the tangential slip from the relative DISPLACEMENT of the closest points (the C3.1 trap), the shipped return map scatters the motion-opposing `tFric` via the edge weights, symmetric tangent (Csl behind `-edgeConsistentTan`); `-edgeMu`/`-edgeKt`/`-edgeCohesion`/`-edgeTauMax`; μ≤0 ⇒ byte-identical. Oracle `proto_e2_penalty.py` 23/23 + `proto_e3_friction.py` 12/12 + `test_adr57_edge_edge_{1,2}` 4/4+4/4 (E2: crossed-bars restrained + NTS-passes-through + oblique-band EE-1 regression + byte-identity; E3: explicit raking Coulomb-opposes-motion + implicit stick + symmetric-solver-safe + μ=0 byte-identity) + full contact battery **149 green**; 3 adversarial gates PASS (E2 folded 2: the **face-mortar stand-down** for the oblique-band double-count + the facet-normal sign anchor; E3 no findings). E4 geom tangent → E5 SOFT → E6 ALM → E7 integration pending ([[57_ladruno_edge_edge_contact_adr]]; predefined-surface only, fenced to 55) | 57 |
