...
| Perpendicular edge-edge (cos_t→0) narrow phase | `SRC/domain/contact/LadrunoEdgeKernel.h` (**E0 shipped**) + `LadrunoContactFE` `EDGE_EDGE` mode (E2) + `LadrunoContactDomain::EdgeEdgeState` (E5+) | 🔄 in progress — **E0 (geometry) SHIPPED** (`LadrunoEdgeKernel.h`: Ericson clamp ladder + common-perp normal w/ body-fixed committed sign + gap + B-operator; oracle 28/28 + `e0_cpp_check.cpp` 13/13 bit-for-bit; header-only ⇒ build byte-identical). **E1 (routing logic) ORACLE SHIPPED** (`proto_e1_router.py` 16/16; the align_fn regime-split precedence — caught oracle-first that flat NTS-precedence steals facing-facet contact; C++ routing folds into E2). E2 penalty force/tangent → E7 integration pending ([[57_ladruno_edge_edge_contact_adr]], design gate PASS-equiv; predefined-surface only, fenced to 55) | 57 |
