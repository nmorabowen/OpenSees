...
| SOFT=1 Courant-stable penalty (explicit) | `LadrunoContactFE::softKn` + `LadrunoContactHandler::ladrunoBuildNodalMass` + `LadrunoContactDomain` nodal-mass cache | ✅ shipped (B1) — `-soft <SOFSCL>` (NTS: RIGID_PLANE + SEGMENT); explicit-only, byte-identical otherwise | 39 (P4) |
| SOFT=2 segment-based penalty (corner/edge) | `LadrunoContactFE::addSoft2Penalty` + `MortarContact.softScale` (reuses `LadrunoMortarKernel` clip→Gauss + the `ladrunoBuildNodalMass` cache) | ✅ shipped (B2) — `contact … -mortar -soft <SOFSCL>`; segment-based explicit penalty catching the corner/edge/T-intersection cases NTS SOFT=1 misses, dt_cr un-throttled; explicit-only (implicit byte-identical), `-soft`-absent byte-identical. **+ viscous SOFT=2** ([#412](https://github.com/nmorabowen/OpenSees/pull/412)): `-mortar -soft -visc <μ_c>` allowed — the D2.2 normal damper on the SOFT=2 active set. **+ frictional SOFT=2** ([#414](https://github.com/nmorabowen/OpenSees/pull/414)): `-mortar -soft -mu/-cohesion/-tauMax` allowed — Courant-stable Coulomb/Tresca via a per-node segment `softKt` + the shipped return map over the soft pressure (`addSoft2Penalty`); only `-tie` still refused; μ=0 byte-identical; battery 114→119/119. SOFT=2 now does viscous + friction | 39 (P5) |
| `∂n/∂u` consistent normal tangent (`-geomtan`) + `ladrunoContactForce` query | `LadrunoContactProjection.h::normalGeomTangent` + `LadrunoContactFE::addNormalGeomTang` | ✅ shipped (B3) — quadratic Newton on curved/large-sliding; SYMMETRIC (solver-safe), off-default (byte-identical) | 39 (P2b-2c) |
...
| Perpendicular edge-edge (cos_t→0) narrow phase | `SRC/domain/contact/LadrunoEdgeKernel.h` (**E0**) + `LadrunoContactFE` `EDGE_EDGE` mode (**E2/E3/E5/E6**) + `LadrunoContactDomain::EdgeEdgeState` (**E2/E6**) + handler routing + `-edgeedge`/`-edgeSoft`/`-edgeAlm` + `ladrunoEdgePenetration` query | ✅ **COMPLETE (E0–E3 + E5 + E6 + E7; only E4 geom-tangent deferred/demand-driven)** — **E0 (geometry)**: `LadrunoEdgeKernel.h` (Ericson clamp ladder + common-perp normal w/ body-fixed committed sign + gap + B-operator; oracle 28/28 + `e0_cpp_check.cpp` 13/13 bit-for-bit). **E1 (routing)** `proto_e1_router.py` 16/16 (the align_fn regime-split precedence — caught oracle-first that flat NTS-precedence steals facing-facet contact). **E2 (penalty force + main tangent)**: a 4th `LadrunoContactFE` mode `EDGE_EDGE=4` (stateless 4-node adapter, **no new class tag**; `f=t_N·B`, `t_N=ε_N⟨−g_N⟩`, `K=ε_N·BᵀB`), Domain `EdgeEdgeState` (composite-key, body-fixed committed sign, **commit()/revert() extended to iterate it**), handler routing over the **brute-force** facet pairing injecting one adapter per kept boundary-edge pair, `-edgeedge` opt-in (OFF ⇒ byte-identical; the EE-1 face-mortar stand-down for the oblique band). **E3 (friction)**: the 4th consumer of the shipped `LadrunoFrictionKernel` (NO new friction code), `edgeSlip` from the relative DISPLACEMENT (the C3.1 trap), `-edgeMu`/`-edgeKt`/`-edgeCohesion`/`-edgeTauMax`; μ≤0 ⇒ byte-identical. **E5 (explicit Courant-stable SOFT)** (#434): under `CentralDifferenceLadruno`, `-edgeSoft <SOFSCL>` replaces ε_N with `k_soft=SOFSCL·4·m_eff/dt²` (the 4-node edge gap-mode mass, the B1 closed form generalized; reuses `ladrunoBuildNodalMass`) ⇒ edge-on impact at the structural dt; residual-only ⇒ implicit byte-identical. Oracle 26/26 + `test_adr57_edge_edge_3` 3/3. **E6 (one-scalar commit-cycle ALM)** (#437): the point-like mortar-C2.2 Uzawa, ONE scalar λ_N/pair (no shared-node accumulator), `p=min(0,λ_N+ε_N·g_N)`, one Uzawa per commit ⇒ ε_N-independent penetration; `getMaxEdgePenetration()` + `ladrunoEdgePenetration` read by the SHIPPED `analyze_augmented` proc UNCHANGED; `-edgeAlm` opt-in, implicit-only, off ⇒ byte-identical. Oracle 24/24 + `test_adr57_edge_edge_4` 3/3. **E7 (integration trust gate)**: `test_adr57_edge_edge_5` 8/8 on REAL `LadrunoBrick` bodies (cross-stacked bars δ=P/εN; NTS-passes-through; SOFT from element-density mass; ALM on a deformable body; a non-axis-aligned crossing n≠ẑ; friction on a real brick; byte-identity at scale; the large-sliding limit as a graceful release). Full contact battery **141 green**; **6 adversarial gates PASS**. The faceted narrow-phase taxonomy (vertex-face NTS + face-face mortar + edge-edge) is now COMPLETE. **E4 (consistent geom tangent, `-edgegeomtan`)** DEFERRED — demand-driven (only helps large-sliding implicit Newton, the fenced regime; can shrink the basin). ([[57_ladruno_edge_edge_contact_adr]]; predefined-surface only, fenced to 55) | 57 |
...
| **B** NTS lane | B1 | P4 SOFT=1 Courant-stable penalty (explicit `dt_cr` not throttled by `kₙ`) | explicit stability + energy balance | ✅ **SHIPPED** — `contact … -soft <SOFSCL>` / `contactPlane … -soft <SOFSCL>` (NTS: RIGID_PLANE + SEGMENT; refused on `-mortar`). Under explicit `CentralDifferenceLadruno` the adapter REPLACES kₙ with the LS-DYNA §26.15 `k_soft = SOFSCL·4·m_eff/dt²` each step (SOFSCL default 0.10) ⇒ the contact's own `ω·dt = 2√SOFSCL ≤ 2`, so explicit dt_cr is NEVER penalty-throttled and impact/pounding runs at the STRUCTURAL dt (the 50_/51_ progressive-collapse enabler). `m_eff = 1/(B M⁻¹ Bᵀ) = 1/(invMproj_slave + Σ Nᵢ²·invMproj_segᵢ)` — the gap-mode generalized mass (fixed master ⇒ ∞ mass ⇒ m_eff=m_s; rigid plane ⇒ m_s). dt reached by `dynamic_cast`→`CentralDifferenceLadruno::getCurrentDeltaT()` (fails for implicit ⇒ SOFT inert ⇒ configured kₙ ⇒ byte-identical). **m_eff uses the ASSEMBLED mass the integrator inverts** (nodal `mass` + Σ element-diagonal mass; `Node::getMass()` holds only the nodal piece) — handler pre-builds a per-node cache when any soft contact exists (off-path byte-identical). Oracle `proto_b1_soft_penalty.py` (coefficient + m_eff closed-form + stable-dt + energy-bounded-SOFSCL-convergent + the stiffness-kn-throttles trade); `test_adr39_contact_p4_soft.py` 7/7 (stiff DIVERGES vs soft BOUNDED; ELEMENT-mass soft-stabilized; e≈1 vs stiff e≫1; implicit `==`; SEGMENT; refusals); battery 91→98/98. **3-reviewer adversarial gate → 1 MAJOR (m_eff from `Node::getMass()` missed element mass) FIXED + 4th-reviewer-verified; energy-label overclaim folded; 1 MINOR (parallel SOE) deferred — serial-only today.** SOFT vs auto-kn = larger penetration for a non-throttled dt (the SOFSCL knob). See [[LEDGER_quirks]] (2 B1 entries). **kt follow-up [#409](https://github.com/nmorabowen/OpenSees/pull/409):** SOFT=1 originally sized only the NORMAL kn — under explicit a stiff friction `kt` still throttled dt_cr via the tangential STICK mode `ω_t=√(kt/m_eff_t)` (continuous stick at a high cone diverges when `ω_t·dt>2`). `LadrunoContactFE::softKt` (+ the extracted `gapModeInvMass`) now sizes the stick penalty Courant-stably `k_soft_t = SOFSCL·4·m_eff_t/dt²` (`m_eff_t` = the worst-case gap-mode mass over the two tangents to n; exact for isotropic lumped mass) in the SEGMENT friction residual under explicit only (implicit / `-soft`-absent byte-identical). Oracle +T6; `test_adr39_contact_p4_soft.py` +2; battery 104→106/106. 2-reviewer gate PASS. |
| | B2 | P5 SOFT=2 segment-based penalty (corner/edge/T-intersection) | corner/edge robustness | ✅ **SHIPPED** — `contact … -mortar -soft <SOFSCL>` selects the SOFT=2 **segment-based** explicit penalty (the segment-to-segment generalization of B1's NTS SOFT=1). The shipped B1 is NODE-to-segment ⇒ it MISSES contact in three regimes — (1) two facets overlapping in an interior strip where NO slave node projects in-bounds, (2) a node sliding off a master segment edge (projection OOB ⇒ pair inert), (3) a T-intersection with no unique owning segment. B2 REUSES the shipped `LadrunoMortarKernel` clip→Gauss (D,M,g̃,n over the slave-facet∩master-facet overlap) and distributes a Courant-stable penalty `p_I = min(0, k_soft,I·ḡ_I)`, `ḡ_I=g̃_I/a_I`, with `k_soft,I = SOFSCL·4·m_eff,I/dt²` and the SEGMENT-BASED gap-mode mass `m_eff,I = 1/(B_I M⁻¹ B_Iᵀ)`, `B_I = [D_IJ/a_I·n | −M_IK/a_I·n]` (the B1 NTS `[n|−Nᵢ n]` operator generalized; reuses the assembled nodal-mass cache) ⇒ each contact mode `ω_I·dt = 2√SOFSCL ≤ 2`, so explicit dt_cr stays un-throttled while CATCHING the cases NTS misses (NTS net force ZERO where no slave node is in-bounds; SOFT=2 integrates the overlap) — the robustness the 50_/51_ progressive-collapse / element-removal direction needs once bodies fragment and contact on arbitrary edges. New `LadrunoContactFE::addSoft2Penalty` + an explicit-only fast path at the TOP of the MORTAR `getResidual` (`softScale>0 && dynamic_cast<CentralDifferenceLadruno*>` ⇒ assemble + early-return; strictly additive ⇒ `softScale=0` byte-identical); under IMPLICIT the cast fails ⇒ falls through to the shipped penalty/ALM with the configured epsN (explicit-only, like B1). Self-equilibrating scatter `f^s=−(D·p)n, f^m=+(Mᵀ·p)n` (the C2 normal block). FRICTIONLESS MVP — `-mortar -soft` ⊥ `-tie/-mu/-cohesion/-tauMax/-visc`; SOFT=1 vs SOFT=2 selected by the formulation. Oracle `proto_b2_soft2_segment.py` (T1 segment m_eff closed-form == `B_I M⁻¹ B_Iᵀ` + matched/fixed-master limits; T2 corner/edge/T NTS-MISSES-SOFT=2-CATCHES; T3 central-difference stability+energy; T4 ASSEMBLED K_c eigenbound `ω_max·dt ≤ 2`). `test_adr39_contact_p5_soft2.py` 6/6 (edge-overlap NTS-passes-through vs SOFT=2-restrained; stiff diverges vs soft bounded; energy restitution; IMPLICIT byte-identical; refusals; bare `-soft`); battery 98→104/104. **3-reviewer adversarial gate → PASS** (no BLOCKER/MAJOR — m_eff matches the oracle, the B1 `Node::getMass()` bug is AVOIDED, byte-identity + sign/KKT + self-equilibrium verified). **1 MINOR FOLDED:** the per-node Courant bound is necessary-not-sufficient — inter-node coupling in the ASSEMBLED K_c raises `ω_max·dt` ~2× (oracle T4: 1.19 vs per-node 0.63 at SOFSCL=0.1; unstable near SOFSCL≈0.3) ⇒ a `-mortar -soft SOFSCL>0.25` warning + a [[LEDGER_quirks]] note; default 0.10 safe. **SOFT=2 vs NTS trade:** segment-based detection is more work per pair (a clip→Gauss integral vs a single projection) but catches the corner/edge/T cases NTS misses — use NTS SOFT=1 for node-dominated contact, SOFT=2 for fragment/arbitrary-edge contact. Follow-ups: frictional/viscous SOFT=2 (both SHIPPED — #412/#414); the perpendicular edge-edge case (cos_t→0 ⇒ the clip degenerates) needs a dedicated edge-edge treatment — **now SCOPED as a design ADR: [[57_ladruno_edge_edge_contact_adr]]** (new narrow-phase algorithm; segment-segment closest point + common-perpendicular normal + the cos_t routing detector; reuses the bucket sort + Domain state + the FE adapter via a new `EDGE_EDGE` mode; implementation is a later session). |
| | B3 | P2b-2c `∂n/∂u` consistent normal tangent + Hertz benchmark | implicit Newton convergence on curved/large-sliding; Hertz `p(r)` | ✅ **SHIPPED** — the GEOMETRIC tangent closes the deferral C2/C3/C4 carried. Pure-fn `LadrunoContactProjection::normalGeomTangent` [+`shapeD2`]: the reduced-Hessian closed form (`H2_αβ=m_αβ−gN·κ_αβ`, `∂ξ̄/∂u=H2⁻¹(Q+gN·P)`, `∂n/∂u=−(P+κ·∂ξ̄)a^γ`), oracle `proto_b3_normal_tangent.py` [analytic==FD ~1e-10 warped quad + tilted large-slide; SYMMETRIC 1e-17; slave-block≡0 flat ⇒ byte-id] + C++ `b3_cpp_check.cpp`. `LadrunoContactFE::addNormalGeomTang` in `addKtToTang`; `Contact.consistentNormal`; command `contact … -geomtan` [off⇒byte-identical `kn·BᵀB`; ⊥`-mortar`]. GATED off-default — SYMMETRIC (solver-safe, ProfileSPD==FullGeneral) but INDEFINITE [`kn·gN·H`, gN<0] ⇒ improves LOCAL convergence (warped-quad slide: **4 vs 7 iters, same root**) but can shrink the global basin on large soft patches (opt-in, like `-consistanttan`). **Gates 1/3/5 ✅** (`test_adr39_contact_p2b2c_hertz.py`: flat byte-identical; curved quadratic; ProfileSPD-safe). **HARNESS:** new `ladrunoContactForce` query (the NTS traction is not in `nodeReaction`) + a converging fixed-rigid-sphere indenter; Hertz `p(r)` is MONOTONE-Hertzian + the patch radius ≈ geometric `√(2Rδ)`, but a quantitative elliptic-`p(r)` / compliant-`a` match is **resolution-bound** by NTS-penalty pressure recovery (master must span many elements to deform half-space-like) ⇒ a finer-mesh study + pairing with displacement-control / D1 augmentation is the follow-up. See [[LEDGER_quirks]] (4 B3 entries). |
...
| **D** enforce/robust | D1 | Commit-cycle ALM mechanics + `analyzeAugmented` Tcl/Py proc (held-load re-commit) | within-step `‖g̃‖→augTol` without recorder/load corruption | ✅ **SHIPPED** ([#411](https://github.com/nmorabowen/OpenSees/pull/411)) — the held-load `analyze_augmented` proc promoted to a FIRST-CLASS no-corruption recipe. It brackets its zero-increment `LoadControl` re-commit loop with new `ladrunoBeginAugment`/`ladrunoEndAugment` commands (Py+Tcl) that set a `Domain::contactAugmenting` flag making `Domain::commit()` SKIP the recorder loop + `commitTag++` (the contact Uzawa λ update + `committedTime=currentTime` STILL run — committedTime a no-op under the held load). So the within-step sweep drives `‖ḡ‖→augTol` (contact) / `‖r̄‖→augTol` (tie) inside ONE physical step with **(1) WITHIN-STEP CONVERGENCE** (penetration εN-independent at finite εN, reached within-step), **(2) NO RECORDER CORRUPTION** (identical sample count + time sequence with/without augmentation — the passes fire no recorders), **(3) NO LOAD CORRUPTION** (the zero-increment `LoadControl` SETS the factor each re-solve via the idempotent `applyLoad`, never accumulates ⇒ converged reaction == single application), **(4) OPT-IN BYTE-IDENTITY** (flag-off ⇒ `Domain::commit()` byte-identical to stock), **(5)** battery 106→**112/112** + new `test_adr41_d1_augmentation.py` (6: within-step contact+tie convergence; recorder no-corruption + an UNBRACKETED negative control; no-load-corruption; opt-in byte-identity), **(6)** the proc folded into a documented first-class recipe (begin/end commands dual-wired Py+Tcl). `try/finally` ⇒ the flag can never stick ON. Oracle `proto_d1_augmentation.py` 11/11. 3-reviewer adversarial gate PASS (2 MINOR folded: begin()+first query() moved inside the try; load gate strengthened with a 10-extra-pass discriminator). **D1 closes the committed contact roadmap (Tracks A→D).** |
| | D2 | Viscous stabilization `-visc μ_c` (`p_visc=μ_c·v_rel`) — funded option for pounding/rocking chatter | chatter/snap-through damped; off under any arc-length lane | ✅ **D2 COMPLETE — D2.1 #385 (NTS: RIGID_PLANE + SEGMENT) + D2.2 #387 (MORTAR contact)** — design [[_adr41_d2_design]]. The velocity-proportional normal damper reuses the penalty operator driven by velocity: `ġ=B·v`, viscous force `f_visc=−μ_c·ġ·B` (in `getResidual` from `getTrialVel`) + the SPD damping tangent `C_visc=μ_c·B Bᵀ` (in `addCtoTang`). Active only in contact (`gap<0` / mortar `p_I<0`); **statics identically inert** (`v≡0` + `StaticIntegrator` never calls `addCtoTang` ⇒ byte-identical to `μ_c=0`). Force-only under mass-only CDL (explicit), force + consistent C tangent under Newmark/HHT (implicit). Command `contact … -visc <μ_c>` / `contactPlane … -visc <μ_c>` (off by default; **REFUSED on `-tie`** — a bond has no contact-chatter regime; works on NTS AND mortar CONTACT). It is a Kelvin–Voigt contact ⇒ exact restitution `e=exp(−ζπ/√(1−ζ²))`, `ζ=μ_c/(2√(kₙ·m))`. **D2.2** added the MORTAR mode: per in-contact node `p_visc_I=μ_c·ḡ̇_I` (`ḡ̇_I=n·(ΣD v_s−ΣM v_m)/a_I`) scattered via D/−M·n like the normal force + the `μ_c·B̃ᵀ diag(W) B̃ ⊗ n⊗n` damping tangent in addCtoTang — the C2 `addMortarTang` block with `epsN→μ_c`. Oracle `proto_d2_viscous.py` (operator FD sym+PSD, static-inert, restitution vs analytic, energy balance 0.005%, active mask; +T6 mortar B̃ operator FD sym+PSD+self-equilibrium). `test_adr41_viscous_d2` 8/8 (explicit-impact Kelvin–Voigt through real CDL, static byte-identical [NTS+mortar], implicit Newmark converges+damps [the C tangent, NTS+mortar], SEGMENT + MORTAR viscous, `-visc`⊥`-tie` refused / `-mortar -visc` accepted); battery 75→83/83. **3 adversarial reviewers total → PASS** (no bug). **D2 COMPLETE (NTS + mortar viscous stabilization).** |
