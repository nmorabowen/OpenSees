...
| | D2 | Viscous stabilization `-visc μ_c` (`p_visc=μ_c·v_rel`) — funded option for pounding/rocking chatter | chatter/snap-through damped; off under any arc-length lane | 🔄 **D2.1 ✅ SHIPPED #TBD (NTS: RIGID_PLANE + SEGMENT)** — design [[_adr41_d2_design]]. The velocity-proportional normal damper reuses the penalty operator driven by velocity: `ġ=B·v`, viscous force `f_visc=−μ_c·ġ·B` (in `getResidual` from `getTrialVel`) + the SPD damping tangent `C_visc=μ_c·B Bᵀ` (in `addCtoTang`). Active only in contact (`gap<0`); **statics identically inert** (`v≡0` + `StaticIntegrator` never calls `addCtoTang` ⇒ byte-identical to `μ_c=0`). Force-only under mass-only CDL (explicit), force + consistent C tangent under Newmark/HHT (implicit). Command `contact … -visc <μ_c>` / `contactPlane … -visc <μ_c>` (off by default; **REFUSED on `-mortar`** — mortar viscous = D2.2). It is a Kelvin–Voigt contact ⇒ exact restitution `e=exp(−ζπ/√(1−ζ²))`, `ζ=μ_c/(2√(kₙ·m))`. Oracle `proto_d2_viscous.py` (operator FD sym+PSD, static-inert, restitution vs analytic, energy balance 0.005%, active mask). `test_adr41_viscous_d2` 6/6 (explicit-impact monotone dissipation + Kelvin–Voigt match through real CDL, static byte-identical, implicit Newmark converges+damps [the C tangent], SEGMENT viscous, `-visc`⊥mortar refused); battery 75→81/81. **2-reviewer adversarial gate → PASS** (no bug). **D2.2 = mortar viscous** (mirror the C2 `addMortarTang` block with `epsN→μ_c`; pull if a mortar chatter gate needs it). |
...
earns a committed phase** — D2.1 (NTS) shipped #TBD as a gated, off-by-default `-visc` option
