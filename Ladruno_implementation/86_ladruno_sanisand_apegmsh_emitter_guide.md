...
> **A cap is only safe under an element that PROPAGATES a material refusal**, and only at the TRIAL. Corrected by WP-99 (F7) — this box used to say "today `LadrunoBrick` only" and named `QuadUP` among the discarders; both were wrong, audited at source on `9c2f964ea`. **Forward any nonzero code (examples):** `LadrunoBrick20`, `LadrunoQuad`/`CST`/`LST`, `LadrunoUP`, `BezierTet10`/`Tri6`, `FourNodeQuad`, `TenNodeTetrahedron`, and **every vanilla u-p element with its own `update()`** — `FourNodeQuadUP` (`:419`), `BBarFourNodeQuadUP` (`:369`), `Nine_Four_Node_QuadUP` (`:572`), `Twenty_Eight_Node_BrickUP` (`:983`). **Forward only the sentinel `LADRUNO_MATERIAL_REFUSED`:** `LadrunoBrick`, the only one (ADR-33/34). **Discard it (examples):** `Brick` (= `stdBrick`; `Brick.cpp:1069` → `return 0` at `:1073`), `BbarBrick`, `BrickUP`, `BBarBrickUP`, `SSPquad`/`SSPquadUP`, `SSPbrick`/`SSPbrickUP`, `FourNodeTetrahedron`, `EnhancedQuad`, `NineNodeMixedQuad`, `LadrunoSolidShell`. **These are EXAMPLES, not the roster** — the full audited table (52 elements) is **Element refusal roster** in `LEDGER_quirks.md` (52 elements: 26 forward, 1 sentinel-only, 25 discard). Under a discarding element a capped update returns early at `T < 1` and the element assembles a **partially-integrated** stress / `alpha` / `fabric` and a partial `aCep_Consistent` as if converged. That is strictly **worse** than the un-capped force-accept it replaces, which at least always drove `T` to 1. **On those elements a capped run is INVALID, not merely un-cut.** At **commit** time no element propagates anything at all (`Domain::commit()` drops the return); under `-implex` the material latches instead (WP-99).
>
