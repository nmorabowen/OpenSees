...
> **A cap is only safe under an element that PROPAGATES a material refusal**, and only at the TRIAL. Corrected by WP-99 (F7) — this box used to say "today `LadrunoBrick` only" and named `QuadUP` among the discarders; both were wrong, audited at source on `9c2f964ea`. **Forward any nonzero code:** `LadrunoBrick20`, `LadrunoQuad`, `LadrunoCST`, `LadrunoLST`, `BezierTet10`, `BezierTri6`, `FourNodeQuad` and **`FourNodeQuadUP`** (`FourNodeQuadUP.cpp:419`, `ret +=` — it propagates). **Forward only the sentinel `LADRUNO_MATERIAL_REFUSED`:** `LadrunoBrick` (ADR-33/34). **Discard it:** `Brick` (= `stdBrick`; `Brick.cpp:1069` → `return 0` at `:1073`), `BbarBrick` (`:951`), `BrickUP` (`:1069`) — these three call `setTrialStrain` inside a *void* `formResidAndTangent` — plus `SSPbrick` (`:445`), `SSPquad` (`:426`) and `LadrunoSolidShell` (`:670`). Under a discarding element a capped update returns early at `T < 1` and the element assembles a **partially-integrated** stress / `alpha` / `fabric` and a partial `aCep_Consistent` as if converged. That is strictly **worse** than the un-capped force-accept it replaces, which at least always drove `T` to 1. **On those elements a capped run is INVALID, not merely un-cut.** At **commit** time no element propagates anything at all (`Domain::commit()` drops the return); under `-implex` the material latches instead (WP-99).
>
