...
- **P5.1 — skeleton + anti-locking (elastic). [BUILT 2026-07-02]** Element class, parser
  ...
  registration surface, G1/G2/G4 (11 gates green; G3 pinched-cylinder/Scordelis-Lo = follow-up
  test PR). Gate discoveries recorded in LEDGER_quirks: the 1-element-thick interior patch must
...
- **P5.2 — state-dependent EAS + nonlinear materials.** `formEAStrue` adaptation (live-σ inner
  Newton, `alphaCommit`, serialization schema v1), `LadrunoConcrete3D`/`LadrunoRCConcrete` hosting,
  D6 lch contract, G5–G6. The adversarial-review pass lands here (novel math: ANS↔EAS interaction,
  softening stability — this is the review hot-spot the fork gate rule exists for).
