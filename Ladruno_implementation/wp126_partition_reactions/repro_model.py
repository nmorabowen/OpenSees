"""WP-126 reproduction: a SUPPORT node shared by two partitions.
...
OUT/<tag>.part-<rank>.* (Ladruno appends .part-N itself when partitioned).
    ...
    serial:  python repro_model.py serial <dir_with_opensees_pyd> <out_dir>
    MP:      mpiexec -n 2 python repro_model.py mp <dir_with_openseesmp_pyd> <out_dir>
"""
...
ops.recorder("ladruno", os.path.join(OUT, f"{tag}_stream.ladruno"), "-N", "reactionForce")
ops.recorder("ladruno", os.path.join(OUT, f"{tag}_env.ladruno"), "-N", "reactionForce", "-envelope")
