...
- **NOT yet verified (next):** the full mesh-objectivity gate — cantilever 1/2/4/8 elements → objective *global* dissipated energy under `-lch ip`, with stock `dispBeamColumn` failing the same gate (negative control). Then Stage-1 (`½θ²` NL strain toggle + Corotational large-disp benchmarks + 3D sibling), then Stage-2 (embedded hinge).
