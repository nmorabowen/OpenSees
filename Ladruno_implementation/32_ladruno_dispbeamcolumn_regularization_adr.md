...
- **NOT yet verified (next):** the headline Tier-1 mesh-objectivity gate with a softening fiber material (`ASDConcrete1D -autoRegularization`): cantilever 1/2/4/8 elements → objective dissipated energy under `-lch ip`, with stock `dispBeamColumn` failing the same gate (the negative control). Then Stage-1 (`½θ²` NL strain toggle + Corotational large-disp benchmarks + 3D sibling), then Stage-2 (embedded hinge).
