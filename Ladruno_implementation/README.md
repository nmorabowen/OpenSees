...
- [[90_ladruno_viscoplastic_regularization_adr]] — **ADR 90 — Duvaut–Lions viscoplastic regularization wrapper**: a generic `NDMaterial` wrapper (`LadrunoDuvautLions`, ND tag 33022 — RESERVED, not yet built) that relaxes any plasticity-type inner material toward its inviscid return, `β = Δt/(τ+Δt)`, to give the SANISAND campaign a **width**-objective localization band (every existing fork gate regularizes *energy* via `lch`, not width). P0 complete: the numpy oracle + A0 bar proved the two-track blend **is** true Duvaut–Lions on proportional-monotonic paths and measured where it is not. See [[_adr90_regularization_planning_brief]] and [[_adr90_a0_results]]. (ADR, P0 complete; P1 wrapper not started)
- [[94_asdplastic_review_plan]] — **ADR 94 — ASDPlasticMaterial3D implementation review**: COMPLETE 2026-09-07 — verdict [[reviews/adr94_verdict]] (#804) + fix wave merged (#806 HB port, #809 fail-loud/parser/Eigen, #815 per-instance state + Voigt convention + apex + relative tol). VM shear vs radial return 1.5e-13. Open: D2 closest-point rewrite (now [[97_ladruno_asdp_closest_point_adr|ADR 97]]), `stdBrick` swallow (by design), StiffSoil coverage.
- [[97_ladruno_asdp_closest_point_adr]] — **ADR 97 — ASDPlasticMaterial3D closest-point return map + consistent tangent**: discharges ADR-94's deferred D2. New opt-in `integration_method Closest_Point` (fully implicit CPPM in `(sigma, q, dlambda)`) plus the previously dead `tangent_type Algorithmic` = its exact consistent tangent — against ADR-94 M3's measured tangent errors (`Continuum` 57 %, the `Secant` default 80 %, `Elastic` 103 %, `Numerical_Algorithmic` 31 % on a third map). `Backward_Euler` stays byte-identical (D1); explicit integrators gated behind `experimental_integrator` (D5). P0 oracles → P1 smooth (VM/DP + apex, AF implicit) → P2 MC/MCTC principal-space multi-surface → P3 HoekBrown → P4 `Numerical_Algorithmic_*` repoint → P5 `experimental_integrator` gate + StiffSoilShear NaN fixes → P6 mesh-scale measurement → P7 closeout. **IMPLEMENTED (opt-in), warrant package [[reviews/adr97_verdict]]**: 23 of 46 specializations (VonMises, Drucker–Prager, MohrCoulomb, MohrCoulombTensionCutoff); `Backward_Euler` + `Secant` stays the shipped DEFAULT and byte-identical (D1); default flip = owner decision, scoped and recommended in the verdict §3, a separate PR.
