...
| **Biot u-p porous element** (`LadrunoUP`) | `element LadrunoUP tag n1..nk matTag <-thick t> -Kf Kf -poro n -rhoF rhof (-perm k1 k2 <k3> \| -permH k1 k2 <k3> -gammaW gw) <-alpha a> <-Ks Ks> <-body ...> <-fluidBody ...> <-formulation std\|bbar> <-pOrder equal\|linear> <-lumped> <-stab auto <a0>\|off\|a> <-dynSeepage on\|off> <-geom linear\|corot\|hypo>` — `(ndm,k)` selects (2,3) T3 · (2,4) Q4 · (2,6) Bézier T6-TH · (3,8) H8 · (3,10) Bézier Tet10-TH; unknown flags FATAL. **`-geom` lanes are narrower than the token list**: `corot` is 3D-only (ADR-78 — a planar node cloud has no polar rotation), `hypo` v1 is the **3D H8 equal-order `std` lane only** (ADR-79 P2), and `finite` is *reserved* (no finite-strain u-p material path, ADR-78 §1.1). Each out-of-lane request is **REJECTED at parse time (fatal), not degraded** — the parser rejects "so the user learns why", so an emitter **can trust a successful parse**: acceptance means the requested lane is the active one, and no client-side `-geom` pre-gate is required. (`upSanitizeGeomMethod` in `LadrunoUP.cpp` *does* downgrade to `linear` with a warning, but it guards direct-ctor construction and `recvSelf` — a streamed `corot`+`ndm==2` would run 3D-indexed loops over 2D buffers — and is **not** on the interpreter path. Do not cite it as emitter-facing behaviour.) | Typed primitive `ops.element.LadrunoUP` (apeGmsh ADR 0074, #793): shape from the fanned-out mesh cell, Gmsh connectivity verbatim, `-pOrder linear` auto-appended on tri6/tet10 (user never types it), pass-through kwargs (None ⇒ not emitted; parser defaults authoritative). **Per-slot ndf inference** (`_ElemSpec.ndf_floor_per_slot`): TH vertices ndf=ndm+1 / mid-edges ndm resolved+validated strictly, `-ndf` tokens emitted/elided vs the envelope — the two-step `model(ndf=…)` dance is gone. Equal-order builder gate rides `_BUILDER_NDF_GATED` (ndm-aware, need=ndm+1). **Build gates**: straight-side pre-check (fork's 1e-6·edge tolerance, `BridgeError` with element/node/offset + hint instead of the setDomain deactivate-and-singularize) and the **general-solver gate** — missing/symmetric `system` refuses to build (allow-list UmfPack/SparseGeneral/FullGeneral/BandGeneral/Mumps; checked per stage; no escape hatch), plus stab-on-TH and non-provider-cell (tet4/prism) refusals with pg context. In `_FORK_ONLY_ELEMENTS`; `ops.capabilities().has_ladruno_up`. Fix/sp/mass/load masks validate against the RESOLVED per-node ndf (a 3-slot p-mask on a 2-dof TH mid-edge node fails loud — target p at vertex-only sets). ELE **33017** (read live). Guide = [[LadrunoUP_guide]]. | **shipped both sides** — fork PRs #551/#557/#559/#563/#566 (v1); apeGmsh #793 (ADR 0074). **Fork-side `-geom` legs shipped AFTER this row was written**: `corot` (ADR-78, #677/#678) and `hypo` (ADR-79 P2, #680) — **apeGmsh-side `-geom` gating for those two lanes is NOT yet shipped** and is the open follow-up. Deferred apeGmsh legs: init-recipe staged verbs, `results.pressure(...)` accessor, P5 shapes (Q9/H20) track the fork. |
...
| **LadrunoCMS partitioned emitter** (ADR-1000 P3a) | driver side: `eigen -ladrunoCMS -domainMode physical -hierarchy logical -level1 <p> -level2 <m> -modesL2 <k2> -modesL1 <k1> <numModes>` with `p*m == nranks` (`-verifyAssembly` is a **parser error** in `physical` mode, not ignored) | **apeGmsh owns the emitter** — a physically partitioned deck (one fragment per MPI rank, owned elements only, nodes on every incident rank, additive mass/loads emitted ONCE on a single primary owner, same global equation id for a shared node on all incident ranks) plus an auditable `cms_partition_manifest.json` (FEMData hash/identity, global counts, `num_partitions`, per-rank element/node counts + sorted hashes, additive owners, shared-node owner sets, `coarse_of_fine` grouping, fragment hashes). Preflight must **refuse** (not warn) on non-exact element coverage, inconsistent owner sets, ownerless/duplicated additive quantities, or any v1-unsupported construction (embedded reinforcement ties, auto structural rebar, `g.embed`, fork contact, zero-length/node-pair explicit elements). Starting point is the existing `g.mesh.partitioning.partition` / `build_element_partition_owner` / `build_node_partition_owners` / `ops.tcl(..., per_rank=True)` path. | **ON HOLD 2026-07-26 — do not schedule. Was production-bound; now likely to be CLOSED — the owner has accepted the speed cost (~7x slower, 5x the memory of sequential ARPACK at Building-1A scale) in exchange for the only modal path where no rank holds the whole problem. apeGmsh side: per-rank emission ALREADY EXISTS (verified at apeGmsh `334cc70e` — owner maps + additive mass/load already emitted once on the primary owner); MISSING is the manifest, `coarse_of_fine`, the CMS preflight refusals, and a second 4-way Building-1A partition** — CMS measured 7.9x slower than the standard solver at n=3200 and 523x at n=12800 (it SCALES WORSE), and the memory-capacity rationale is undercut twice over: `eigen` over `MumpsParallelSOE` already distributes the factor without CMS, and Craig-Bampton's dense constraint-mode block grows faster than the sparse fill it avoids. Only remaining ask: a second 4-way Building-1A partition (data, not code). Full context + stand-down notice in [[LadrunoCMS_apegmsh_emitter_guide]] §0 |
...
- 2026-08-05 — **CORRECTION to the `LadrunoUP` `-geom` note added earlier the
  ...
  must gate `-geom` itself. **Both halves were wrong**, and the conclusion was
  backwards. Verified against source: `OPS_LadrunoUP.cpp` rejects every
  out-of-lane request as a FATAL parse error (2D `corot`, 2D `hypo`, `hypo` with
  `bbar`/TH/non-H8, `finite`, unknown token) — its own comment reads "reject
  rather than degrade so the user learns why". The degrade-to-`linear` lives in
  `upSanitizeGeomMethod` (`LadrunoUP.cpp`), which backs **direct-ctor
  construction and `recvSelf`** so a streamed `corot`+`ndm==2` cannot run
  3D-indexed loops over 2D-sized buffers; it is not reachable from the
  interpreter. Net effect for apeGmsh: **no client-side `-geom` pre-gate is
  needed** — a successful parse is proof the requested lane is active. The
  source-side usage comment was stale in the same way and is fixed in the same
  PR.
...
- 2026-07-13 — Added the **Biot u-p porous element** (`LadrunoUP`, ELE 33017)
  row: the apeGmsh consumer shipped (apeGmsh ADR 0074 / #793) — typed
  `ops.element.LadrunoUP` with shape-from-mesh fan-out, per-slot ndf
  inference for the Taylor–Hood shapes, and the two build gates the fork
  cannot enforce itself (straight-side pre-check; general-solver
  requirement — the ProfileSPD default silently drops a coupling block).
  *(Row merged 2026-08-05, 22 days after it was written; the `-geom` token
  was refreshed at merge time to cover the ADR-78 `corot` and ADR-79 P2
  `hypo` legs that shipped in the interim.)*
...
- 2026-06-29 — Added the **Contact** row (`contactSurface`/`contact`/`contactPlane`
  ...
  absent from this table; apeGmsh now consumes it via `g.constraints.contact`
  (NTS + mortar, face-to-face friction fully exposed, `-soft`/`-visc`/
  `-consistanttan`/`-geomtan` extensions, the `-cell` broad-phase knob),
  `g.constraints.mortar` (deprecated tie alias), and `g.constraints.contact_plane`
  (rigid analytical plane) — auto-emitting the `LadrunoContact` handler,
  serial-only (partitioned emit fails loud), neutral-`model.h5` round-trip via
  `/contacts` + `/contact_planes`. apeGmsh #722/#741/#744/#745/#746/#748/#760/#761.

- 2026-06-29 — **Edge-edge lane exposed** (apeGmsh #766). The mortar-only
  perpendicular edge-edge fallback (`-edgeedge` + the `-edge*` knobs, ADR-57
  E2–E7) now rides `g.constraints.contact(edge_edge=, edge_kn=, edge_band=,
  edge_mu=, edge_kt=, edge_cohesion=, edge_tau_max=, edge_consistent_tan=,
  edge_soft=, edge_alm=, edge_aug_tol=)`. `ContactDef` mirrors the fork's two
  fail-loud gates (`edge_edge` requires `formulation="mortar"`; the `edge_*`
  params require `edge_edge=True`); H5 round-trips via additive edge columns on
  `contact_payload_dtype` (neutral schema 2.24.0 → 2.25.0, presence-probed).
  **The whole contact subsystem is now exposed apeGmsh-side** — no substantial
  contact gap vs the fork remains.
