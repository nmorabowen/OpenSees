...
| **Biot u-p porous element** (`LadrunoUP`) | `element LadrunoUP tag n1..nk matTag <-thick t> -Kf Kf -poro n -rhoF rhof (-perm k1 k2 <k3> \| -permH k1 k2 <k3> -gammaW gw) <-alpha a> <-Ks Ks> <-body ...> <-fluidBody ...> <-formulation std\|bbar> <-pOrder equal\|linear> <-lumped> <-stab auto <a0>\|off\|a> <-dynSeepage on\|off> <-geom linear>` — `(ndm,k)` selects (2,3) T3 · (2,4) Q4 · (2,6) Bézier T6-TH · (3,8) H8 · (3,10) Bézier Tet10-TH; unknown flags FATAL | Typed primitive `ops.element.LadrunoUP` (apeGmsh ADR 0074, #793): shape from the fanned-out mesh cell, Gmsh connectivity verbatim, `-pOrder linear` auto-appended on tri6/tet10 (user never types it), pass-through kwargs (None ⇒ not emitted; parser defaults authoritative). **Per-slot ndf inference** (`_ElemSpec.ndf_floor_per_slot`): TH vertices ndf=ndm+1 / mid-edges ndm resolved+validated strictly, `-ndf` tokens emitted/elided vs the envelope — the two-step `model(ndf=…)` dance is gone. Equal-order builder gate rides `_BUILDER_NDF_GATED` (ndm-aware, need=ndm+1). **Build gates**: straight-side pre-check (fork's 1e-6·edge tolerance, `BridgeError` with element/node/offset + hint instead of the setDomain deactivate-and-singularize) and the **general-solver gate** — missing/symmetric `system` refuses to build (allow-list UmfPack/SparseGeneral/FullGeneral/BandGeneral/Mumps; checked per stage; no escape hatch), plus stab-on-TH and non-provider-cell (tet4/prism) refusals with pg context. In `_FORK_ONLY_ELEMENTS`; `ops.capabilities().has_ladruno_up`. Fix/sp/mass/load masks validate against the RESOLVED per-node ndf (a 3-slot p-mask on a 2-dof TH mid-edge node fails loud — target p at vertex-only sets). ELE **33017** (read live). Guide = [[LadrunoUP_guide]]. | **shipped both sides** — fork PRs #551/#557/#559/#563/#566 (v1); apeGmsh #793 (ADR 0074). Deferred apeGmsh legs: init-recipe staged verbs, `results.pressure(...)` accessor, P5 shapes (Q9/H20) track the fork. |
