...
| `implexDetail` | 6 | `[0]` total error · `[1]` deviatoric leg · `[2]` volumetric leg (`sqrt(3)\|dp\|`) · `[3]` `p_min` clamp fired on the last pass (0/1) · `[4]` clamp fire count, ever · `[5]` `f`, frozen for this step (reads `0` on a guarded step, §11) | `implexDetail_total`, `implexDetail_dev`, `implexDetail_vol`, `implexDetail_clampFired`, `implexDetail_clampCount`, `implexDetail_f` |
...
| `implexGuards` | 5 | `[0]` floor fallbacks (P2-1, `-implexFloor implicit`) · `[1]` guard firings (P2-2, `f = 0` after a reversal/softening commit) · `[2]` holds preserved (P2-3, zero-`dt` commits left alone) · `[3]` reversal resets restored (P2-5, `-reversalTol`) · `[4]` trial-time `f = 0` fallbacks (P2-6, `-implexTrialGuard`) | (none yet — out of WP-86d's scope) |
