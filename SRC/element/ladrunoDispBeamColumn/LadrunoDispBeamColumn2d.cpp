...
void* OPS_LadrunoDispBeamColumn2d()
{
    ...
    // (characteristic) length reported to crack-band materials.
        ...
        if (strcmp(lchArg, "ip") == 0) {
            ...
            userLch = atof(lchArg);
            if (userLch <= 0.0) {
                opserr << "WARNING LadrunoDispBeamColumn2d: -lch value must be ip|element|<positive value>, got '"
                       << lchArg << "'\n";
                return 0;
            }
            lchMode = 2;
...
void* OPS_LadrunoDispBeamColumn2d(const ID &info)
{
    // data
    int iData[5];
    int numData;
...
int OPS_LadrunoDispBeamColumn2d(Domain& theDomain, const ID& elenodes, ID& eletags)
{
    if(OPS_GetNumRemainingInputArgs() < 2) {
	...
	theEle = new LadrunoDispBeamColumn2d(--currTag,elenodes(2*i),elenodes(2*i+1),secTags.Size(),
