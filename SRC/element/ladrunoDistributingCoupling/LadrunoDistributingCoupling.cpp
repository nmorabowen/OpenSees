...
LadrunoDistributingCoupling::LadrunoDistributingCoupling(int tag, int ndm_, int refNode,
  ...
  : LadrunoUndampedElement(tag, ELE_TAG_LadrunoDistributingCoupling),
...
LadrunoDistributingCoupling::LadrunoDistributingCoupling()
  : LadrunoUndampedElement(0, ELE_TAG_LadrunoDistributingCoupling),
...
// setRayleighDampingFactors / getDamp: inherited from LadrunoUndampedElement (WP-123).
...
Response* LadrunoDistributingCoupling::setResponse(const char** argv, int argc, OPS_Stream& s)
{
  ...
  if (LadrunoResp::is(argv[0], "rotationAxes"))
...
int LadrunoDistributingCoupling::getResponse(int responseID, Information& eleInfo)
{
  ...
  case 10: {  // artificial penalty energy 1/2 (Kt|g_t|^2 + Kr|g_r|^2)
    Vector g(nGap); this->computeGap(g);
  ...
  case 11: {  // constraint violation ||g||
    Vector g(nGap); this->computeGap(g);
    double g2 = 0.0;
    for (int k = 0; k < nGap; k++) g2 += g(k) * g(k);
