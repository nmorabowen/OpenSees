...
#define MAT_TAG_LadrunoBondSlip 33002 // N. Mora-Bowen (Ladruno) — 1D bond-slip tau-s (CEB-FIP MC2010) for the embedded-rebar element; ladruno private uniaxial band >=33000
#define MAT_TAG_LadrunoCohesiveHinge 33003 // N. Mora-Bowen (Ladruno) — discrete cohesive moment-rotation law M([[theta]]) carrying Gf per hinge for the LadrunoDispBeamColumn Tier-2 embedded strong-discontinuity hinge (ADR 32 §Tier-2); rigid-softening penalty (guarded floor Mc^2/2Gf) + exp/linear envelope calibrated to int M d[[theta]]==Gf; ladruno private uniaxial band >=33000
...
#define ND_TAG_LogStrain2D 33016 // N. Mora-Bowen (Ladruno) — 2D (plane) Hencky finite-strain adaptor (ADR 25 P5); order-3 PlaneStrain/PlaneStress face over a 3D order-6 inner via an internal LogStrainNDMaterial (F33=1 plane-strain / F33-Newton plane-stress condensation); ladruno private ND band >=33010
...
#define ND_TAG_LadrunoSANISAND 33019 // N. Mora-Bowen (Ladruno) — thin subclass of ManzariDafalias (Ghofrani/Arduino, U.Washington, after Dafalias & Manzari 2004) that makes the two low-stress constants m_Presidual/m_Pmin optional constructor args, carried on the wire, and echoed at construction (defaults 0.0 and 1.0e-3*P_atm, per NTUASand02); vanilla ManzariDafalias not edited; ADR 86 PR-1. Numerically equals LADRUNO_TAG_ComplexEigen 33019 — different registry, deliberately NOT a collision (see classTags.h:592, :643, :945-946 for the same convention)
#define ND_TAG_LadrunoSANISAND3D 33020 // N. Mora-Bowen (Ladruno) — 3D wrapper for LadrunoSANISAND (mirrors ManzariDafalias3D); ADR 86 PR-1. Numerically equals ELE_TAG_LadrunoSolidShell 33020 — different registry, deliberately NOT a collision
#define ND_TAG_LadrunoSANISANDPlaneStrain 33021 // N. Mora-Bowen (Ladruno) — PlaneStrain wrapper for LadrunoSANISAND (mirrors ManzariDafaliasPlaneStrain); ADR 86 PR-1. Numerically equals ELE_TAG_LadrunoCSTPair 33021 — different registry, deliberately NOT a collision
#define LADRUNO_TAG_ComplexEigen 33019 // N. Mora-Bowen (Ladruno) — complex/state-space modal analysis driver (project M,C,K onto the real undamped basis, solve the 2p×2p QZ pencil via dggev); analysis-side object, not a MovableObject — tag reserved per-registry so siblings never collide; ADR 46. Numerically equals ND_TAG_LadrunoSANISAND 33019 — different registry, deliberately NOT a collision (ADR 86)
...
#define ELE_TAG_LadrunoDispBeamColumn2d    33013 // N. Mora-Bowen (Ladruno) — regularized displacement-based fiber frame (2D); Tier-1 per-IP characteristic length (lch) channel mirroring ForceBeamColumn so crack-band/auto-regularizing materials regularize softening over the localizing IP, not the whole element (fixes LEDGER_quirks §59); embedded softening hinge planned (Tier-2); Element-registry tag (numerically equals ND_TAG 33013, different registry — not a collision); ladruno private band >=33000 (see Ladruno_implementation/32_ladruno_dispbeamcolumn_regularization_adr.md)
#define ELE_TAG_LadrunoDispBeamColumn3d    33014 // N. Mora-Bowen (Ladruno) — 3D sibling of LadrunoDispBeamColumn2d; Tier-1 per-IP lch channel + -nl ½(θy²+θz²) biaxial bowing (DispBeamColumnNL3d); Element-registry tag (numerically equals ND_TAG 33014, different registry — not a collision); ladruno private band >=33000 (see Ladruno_implementation/32_ladruno_dispbeamcolumn_regularization_adr.md)
#define ELE_TAG_LadrunoRigidBody           33015 // N. Mora-Bowen (Ladruno) — rigid-body element (ADR 58, D1(b) Element route): zero-stiffness Element owning a PRIVATE INTERNAL 6-DOF CoM Node + MP-slaving of a node set (Joint3D pattern); condensed diagonal translational mass on the internal node; explicit-only; P1 = translation/ballistic (R=I), SO(3) rotation self-integrated in commitState at P2; ladruno private band >=33000 (see Ladruno_implementation/58_ladruno_rigid_body_adr.md)
#define ELE_TAG_LadrunoLST                 33016 // N. Mora-Bowen (Ladruno) — 6-node linear-strain triangle (T6, ADR 70 P3): quadratic N / linear strain, 3-pt interior rule (reduce-to SixNodeTri anchor); std x -geom {linear|finite} on the shared LadrunoFiniteStrain2DKernel. NO bbar/F-bar: constant element-mean dilatation is RANK-DEFICIENT on the T6 (2 conformal zero-energy modes — refuted at P3; volumetric cure = ADR 70 P4). ELE registry, distinct from ND_TAG_LogStrain2D=33016; ladruno private band >=33000 (see Ladruno_implementation/70_ladruno_plane_finite_triangles_adr.md)
#define ELE_TAG_LadrunoUP                  33017 // N. Mora-Bowen (Ladruno) — unified Biot u-p saturated-porous continuum element (ADR 71): honest pore-pressure nodal DOF (per-node [u..., p], p IS p — not upstream's ∫p·dt rate trick), internal stateless shape providers (P1: T3/Q4/H8 equal-order + McGann α-stabilization; P3: Bézier T6/Tet10 Taylor–Hood), -formulation {std|bbar}; UNSYMMETRIC tangent (general solver required). Numerically equals ND_TAG 33017 (LadrunoConcrete3D), ND_TAG 33018 (LadrunoRCFiniteStrain), LADRUNO_TAG 33019 (ComplexEigen) — per-registry tags, not collisions. ladruno private band >=33000 (see Ladruno_implementation/71_ladruno_up_family_adr.md)
#define ELE_TAG_LadrunoBrick20             33018 // N. Mora-Bowen (Ladruno) — 20-node serendipity quadratic hex (H20), -formulation {std|uri}: 27-pt full (reduce-to Twenty_Node_Brick anchor) / 2x2x2 uniform reduced (C3D20R analog, Barlow points, no hourglass control); HRZ-only -lumped (row-sum corner masses are -M/8); ladruno private band >=33000 (see Ladruno_implementation/72_ladruno_second_order_brick_adr.md). 33019 pencilled for the future H27 sibling.
#define ELE_TAG_LadrunoSolidShell          33020 // N. Mora-Bowen (Ladruno) — 8-node ANS/EAS solid-shell (ADR 66): 3-translational-DOF punching/bearing specialist carrying the full triaxial sigma_33 through shell geometry; Dvorkin-Bathe ANS transverse shear + Betsch-Stein ANS E33 + state-dependent EAS-on-E33 (committed alpha, adapted LadrunoBrick formEAStrue); 2x2 in-plane x selectable nz gauss/lobatto; in-plane-projected lch; NOT a co-equal flexural host. 33020 honored as reserved by ADR 19 — of 33016-33019: 33016 reserved (LadrunoLST, ADR 70 P3), 33017 = LadrunoUP (ADR 71), 33018 = LadrunoBrick20 (ADR 72), 33019 pencilled for H27 — none free; ladruno private band >=33000 (see Ladruno_implementation/66_ladruno_solidshell_adr.md). Numerically equals ND_TAG_LadrunoSANISAND3D 33020 (ADR 86) — different registry, deliberately NOT a collision
#define ELE_TAG_LadrunoCSTPair             33021 // N. Mora-Bowen (Ladruno) — disjoint 2-triangle F-bar-Patch macro-element (ADR 70 P4a, dSNPO 15.1.9): 4-node CCW quad split along the n1-n3 diagonal, both T3s share the patch dilatation J-bar = v_patch/V_patch (F-bar_e = (J-bar/J_e)^{1/2} F_e); exact patch-local UNSYMMETRIC consistent tangent (eqs 15.37/15.38, stress-proportional cross blocks) on the shared LadrunoFiniteStrain2DKernel via 4-node padded rows; finite-strain + PlaneStrain only (needs FiniteStrainND2DMaterial, e.g. LogStrain2D); the T3 volumetric cure the P4 design spike decided; ladruno private band >=33000 (see Ladruno_implementation/70_ladruno_plane_finite_triangles_adr.md §9). Numerically equals ND_TAG_LadrunoSANISANDPlaneStrain 33021 (ADR 86) — different registry, deliberately NOT a collision
...
#define INTEGRATOR_TAGS_ExplicitBathe                   33000 // J.A. Abell (Ladruno) — explicit Bathe integrator; ladruno private band >=33000 (was 61, one above upstream frontier HarmonicSteadyState=60). ADR-52 W1-E2: this ONE class now hosts the whole ExplicitBathe* family via orthogonal -lnvd/-sms/-consistent flags; the flag combo derives the classTag (tagForFlags) so 33002/33009/33010/33011/33012 are DEPRECATED-but-recognized aliases (do NOT free/reuse — saved-DB/parallel recvSelf routes them to ExplicitBathe::makeForBroker).
...
#define INTEGRATOR_TAGS_ExplicitBatheLNVD               33002 // N. Mora-Bowen (Ladruno) — Noh-Bathe + LNVD damping; ladruno private band >=33000 (was 63). ADR-52 W1-E2: DEPRECATED alias of the collapsed ExplicitBathe (-lnvd); tag retained for serialization (recvSelf -> ExplicitBathe::makeForBroker), do NOT free/reuse.
...
#define INTEGRATOR_TAGS_CentralDifferenceSMSConsistent   33008 // N. Mora-Bowen (Ladruno) — CONSISTENT (Olovsson) selective mass scaling on CentralDifferenceLadruno (v2 of CentralDifferenceSMS=33007); centroidal scaling mass M_bar=beta[diag(m)-mm^T/M_e] + matrix-free PCG solve of M_tilde a=r; ladruno integrator band >=33000. NB 33008 is independently reused in the ELE_TAG space (ELE_TAG_LadrunoCST=33008) — tag bands are PER-REGISTRY, no collision. See Ladruno_implementation/38_ladruno_consistent_mass_scaling_adr.md.
#define INTEGRATOR_TAGS_ExplicitBatheSMS                 33009 // N. Mora-Bowen (Ladruno) — LUMPED selective mass scaling on the Noh-Bathe ExplicitBathe (sibling of CentralDifferenceSMS=33007); additive nodal mass via the shared Ladruno::buildMassScaling, no solve-path change (RHS mass assembly sees it); ladruno integrator band >=33000. See Ladruno_implementation/36_ladruno_selective_mass_scaling_adr.md. ADR-52 W1-E2: DEPRECATED alias of the collapsed ExplicitBathe (-sms); tag retained for serialization (recvSelf -> ExplicitBathe::makeForBroker), do NOT free/reuse.
#define INTEGRATOR_TAGS_ExplicitBatheSMSConsistent       33010 // N. Mora-Bowen (Ladruno) — CONSISTENT (Olovsson) selective mass scaling on the Noh-Bathe ExplicitBathe (sibling of CentralDifferenceSMSConsistent=33008); centroidal M_bar + matrix-free PCG refined at BOTH Noh-Bathe sub-step solves via the base refineAccel hook; ladruno integrator band >=33000. See Ladruno_implementation/38_ladruno_consistent_mass_scaling_adr.md. ADR-52 W1-E2: DEPRECATED alias of the collapsed ExplicitBathe (-sms -consistent); tag retained for serialization (recvSelf -> ExplicitBathe::makeForBroker), do NOT free/reuse.
#define INTEGRATOR_TAGS_ExplicitBatheLNVDSMS             33011 // N. Mora-Bowen (Ladruno) — LUMPED selective mass scaling on the Noh-Bathe + FLAC-LNVD ExplicitBatheLNVD (sibling of ExplicitBatheSMS=33009); additive nodal mass via the shared buildMassScaling, no solve-path change; ladruno integrator band >=33000. See Ladruno_implementation/36_ladruno_selective_mass_scaling_adr.md. ADR-52 W1-E2: DEPRECATED alias of the collapsed ExplicitBathe (-lnvd -sms); tag retained for serialization (recvSelf -> ExplicitBathe::makeForBroker), do NOT free/reuse.
