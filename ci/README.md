...
| `check_quirk_patterns.py` | `LEDGER_quirks` entries with a greppable pattern, enforced on fork-stamped sources: L1 Rayleigh snapshot (#562), L2 singleton reset on `wipe`, L3 task-guide pointers resolve (WP-115), L4 an Element subclass's `commitState()` chains to `Element::commitState()` so `betaKc`'s Kc is refreshed (WP-118), L5 no element subtracts its load vector in both `getResistingForce()` and the `getResistingForceIncInertia()` that calls it — the ground-motion load counted twice (WP-119; scans ALL element files, vanilla included). Self-test: `test_check_quirk_patterns.py` | none | 1 on any unwaived finding |
